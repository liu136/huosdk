...
class PasswordController extends MobilebaseController {
    ...
    public function uppwd_post() {
        if (IS_POST) {
            ...
            if (!empty($_SESSION['uppwd_token']) && $_SESSION['uppwd_token'] == $action) {
                ...
                $userdata = $this->member_model->where(
                    array(
                        'id' => $mem_id
                    )
                )->find();
                if ($data['password'] == $userdata['password']) {
                    ...
                    $rs = $this->member_model->where(
                        array(
                            'id' => $mem_id
                        )
                    )->save($userdata);
                    if (false != $rs) {
                        $this->success("密码修改成功", U('Password/pwd_success'), 0);
                        exit();
                    }
                } else {
                    $this->error("原密码错误");
    ...
    public function pwdconfirm_post(){
          if (IS_POST) {
             $is_action = I('post.action');
              if(!$is_action){
                  exit(json_encode(array('status'=>0,'msg'=>'')));
              }
              $_SESSION['uppwd_token'] = md5(uniqid());
              exit(json_encode(array('status'=>1,'msg'=>$_SESSION['uppwd_token'])));
          }else{
            exit(json_encode(array('status'=>0,'msg'=>'')));
          }
