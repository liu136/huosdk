...
class WalletController extends MobilebaseController {
    ...
    public function index() {
        $this->assign('title', C('CURRENCY_NAME'));
        $this->display();
    }
    ...
    public function charge_detail() {
        ...
        $this->assign('title', C('CURRENCY_NAME')."充值记录");
        $this->display();
    }
    ...
    public function pay_detail() {
        ...
        $this->assign('ptbpaytitle', C('CURRENCY_NAME')."消费记录");
...
//        }
        $pay_controller = new \Common\Controller\PaybaseController();
        $userid = $_SESSION['user']['id'];
        $ptb_info = $pay_controller->getUserwallet($userid);
        $this->assign('wallet', $ptb_info);
//        $pay_controller = new \Pay\Controller\WalletpayController();
        //获取用户agentid
        $agentid = M("members")->where(array("id" => $userid))->getField("agent_id");
        $ratedata = 1;
...
//        $this->assign("gamedata", $gamedata);
    ...
    public function ptb_preorder() {
        $pay_controller = new \Pay\Controller\WalletpayController();
        $pay_data = $pay_controller->ptb_preorder();
        $data['payway'] = I('post.paytype/s', '');
        if (empty($pay_data)) {
            $data['status'] = 0;
            $data['token'] = '';
