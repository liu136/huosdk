...
class ForgetpwdController extends HomebaseController {
    ...
    public function check() {
        ...
        $_email_controller = new \Common\Controller\EmailController();
        $_is_email = $_email_controller->checkEmail($username);
        $_is_mobile = $sms_controller->checkPhone($username);
        if($_is_mobile){
            $map['bindmobile'] = $username;
        }else if ($_is_email){
            $map['bindemail'] = $username;
        }else{
