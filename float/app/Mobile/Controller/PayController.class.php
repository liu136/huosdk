...
class PayController extends MobilebaseController {
    ...
    public function pay_json() {
        ...
        $where['p.mem_id'] = sp_get_current_userid();
        $field
            = "p.amount money, p.order_id order_id, g.name gamename,DATE_FORMAT(FROM_UNIXTIME(p.create_time),'%Y-%m-%d %T') c_time";
    ...
    public function charge_json() {
        ...
        $where['gc.mem_id'] = sp_get_current_userid();
        $field
            = "gc.money,gc.order_id, gm.payname gmname,DATE_FORMAT(FROM_UNIXTIME(gc.create_time),'%Y-%m-%d %T') c_time";
    ...
    public function yxb_json($option = null) {
        ...
        $where = "gmm.mem_id = ".sp_get_current_userid();
        $time = time();
        if (2 == $option) {
            $where .= " AND gmm.update_time+gm.expire_time >".$time;
        }
        $field
            = "gmm.remain gmcnt, DATE_FORMAT(FROM_UNIXTIME(gmm.update_time+gm.expire_time),'%Y-%m-%d %T') expiretime ";
