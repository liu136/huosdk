...
class SmsController extends AppframeController {
     ...
     * 发送手机校验码，登陆注册时
     */
    public function send_auth_code($phone, $type, $expire_time = 0) {
        // 检查手机号码合法性
        ...
        if (empty($expire_time)) {
            ...
            $limit_time = $expire_time; // 设定超时时间 2min
        }
        // 先判断是否发送过验证码
    ...
    public function sms_verify_code($phone, $code, $expire_time = 0) {
        if (empty($expire_time)) {
            $limit_time = 120;
        } else {
            $limit_time = $expire_time; // 设定超时时间 2min
    ...
    public function send_sms_code($phone, $code, $type) {
        ...
        if (1 == $sendtype) {
            ...
            $al_rs = $this->send_junlong_sms_code($phone, $code, $type);
        } else {
            $al_rs = $this->send_alidayu_sms_code($phone, $code, $type);
        }
     ...
     * @return boole 是否发送成功
     */
    private function send_junlong_sms_code($phone, $code, $type) {
        // 获取君隆短信配置信息
        if (file_exists(SITE_PATH."conf/sms/junlong.php")) {
            $_config = include SITE_PATH."conf/sms/junlong.php";
        } else {
            return false;
        }
        if (empty($_config)) {
