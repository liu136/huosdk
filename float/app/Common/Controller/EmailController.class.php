...
class EmailController extends AppframeController {
     ...
     * 发送邮箱校验码，登陆注册时
     */
    public function send_auth_code($email, $type, $expire_time = 0, $username) {
        // 检查邮箱合法性
        ...
        if (isset($_SESSION['email']) && $_SESSION['email'] == $email
            && $_SESSION['email_time'] + $limit_time > time()
        ) {
            $result['status'] = 1;
    ...
    public function verify_code($email, $code, $expire_time = 0) {
        if (empty($expire_time)) {
            $limit_time = 120;
        } else {
            $limit_time = $expire_time; // 设定超时时间 2min
