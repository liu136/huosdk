...
//                 $param['sign'] = (string) $sign;
     ...
     * 平台币支付通知函数
     */
    protected function ptb_wallet_post($orderid, $amount, $paymark = '') {
        ...
        if (($myamount == $amount) && 2 != $pay_data['status']) {
            ...
            $rs = $pay_model->save($pay_data);
            if ($rs) {
                $this->updatePtb($pay_data);
            }
        }
    }

    private function updatePtb($paydata) {
        $pm_model = M('ptb_mem');
        $map['mem_id'] = $paydata['mem_id'];
        $pm_data = $pm_model->where($map)->find();
        if (empty($pm_data)) {
