...
class MobilebaseController extends AppframeController {
    ...
    function _initialize() {
        ...
        if (empty($_SESSION['user'])) {
            $user_token = I('post.user_token/s', '');
            if (!empty($user_token)) {
                //SDK验证
                ...
                $p_token = I('param_token/s', '');
                if ($param_token == $p_token) {
                    $_SESSION['user'] = M('members')->where(array('id' => $_SESSION['mem_id']))->find();
                }
            } else {
                ...
                if ($this->mem_id != $_SESSION['user']['id']) {
                    session(null);
                }
            ...
            $user_token = I('post.user_token/s', '');
            $head = $this->get_header('hs-token');
            $this->client_id = I('param.clientid/d', 0);
            if (!empty($user_token)) {
                if ($_SESSION['mem_id'] != $_SESSION['user']['id']) {
                    $_SESSION['user'] = M('members')->where(array('id' => $_SESSION['mem_id']))->find();
                }
            } else if (!empty($head) && !empty($this->client_id)) {
                $this->agent = I('param.agent/s', 'default');
                $this->agent_id = $this->getAgentid($this->agent);
                $this->app_id = I('param.appid/d', 0);
                $this->client_key = $this->getClientkey($this->client_id);
