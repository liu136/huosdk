...
class TagLibFloat extends TagLib {
    /**
     *
     * @var type 标签定义：
     *      attr 属性列表
     ...
     * @return array
     */
    public function _floattpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "Float"."/".$tag['file'];
        } else {
        ...
        $TemplatePath = SITE_PATH.$TemplatePath;
        // 判断模板是否存在
