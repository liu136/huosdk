...
if (file_exists(SITE_PATH."conf/company.php")) {
    $company = include SITE_PATH."conf/company.php";
} else {
    $company = array();
}
...
$configs = array(
    ...
    'SP_TMPL_PATH'                 => SITE_PATH.'float/themes/',  // 前台模板文件根目录
    ...
    'SP_ADMIN_TMPL_PATH'           => SITE_PATH.'float/themes/',       // 各个项目后台模板文件根目录
    'SP_ADMIN_DEFAULT_THEME'       => 'web',       // 各个项目后台模板文件
    'SP_ADMIN_TMPL_ACTION_ERROR'   => 'Mobile/error.html', // 默认错误跳转对应的模板文件,注：相对于后台模板路径
    'SP_ADMIN_TMPL_ACTION_SUCCESS' => 'Mobile/success.html', // 默认成功跳转对应的模板文件,注：相对于后台模板路径
...
return array_merge($configs, $comconf, $db, $runtime_config, $company, $setting_config);
