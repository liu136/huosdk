...
class WalletpayController extends MobilebaseController {
    ...
    public function preorder() {
        ...
        switch ($pw_id) {
            ...
            case 3 : {
                $spay_controller = new AlipayController();
                $rdata = $spay_controller->mobilePay($data);
//                $orderdata['order_id'] = $data['order_id'];
//                $orderdata['productname'] = '游戏币';
//                $orderdata['productdesc'] = '游戏币充值';
//                $orderdata['real_amount'] = $data['real_amount'];
...
//                $re = fopen($dir, 'a');
//                fwrite($re, json_encode(array($rdata['payinfo'])));//将内容写入文件
//                fwrite($re, "\r\n");//将内容写入文件
//                fclose($re);
...
//                     $rdata['notify_url'] = SDKSITE . U("Pay/Alipay/alipay_walletnotify");
    ...
    public function ptb_preorder() {
        ...
        $data = $this->ptb_wallet_pay($amount, $pw_id, $app_id);
        switch ($pw_id) {
            case 1 : {
                break;
            }
            ...
            case 3 : {
                $spay_controller = new AlipayController();
                $rdata = $spay_controller->ptb_mobilePay($data);
                return $rdata;
                break;
            }
            case 4 : {
    ...
    private function ptb_wallet_pay($amount, $pw_id, $app_id) {
        if (empty($amount)) {
            $this->error("充值金额不正确，请输入整数");
        }
        if ($pw_id <= 0) {
...
//        }
        ...
        $data['real_amount'] = $amount * $discount;
        $real_cnt = $data['gm_cnt'] + $data['rebate_cnt'];
        $data['discount'] = round($data['real_amount'] / $real_cnt, 2);
        $data['payway'] = $payway;
        $data['ip'] = get_client_ip();
        $data['status'] = 1;
        $data['create_time'] = time();
