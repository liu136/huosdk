...
function ChkPwd() {
    ...
    if (pwd_len < 6) {
        ShowMsg("pwdspan", 0, "6-16位的字符!由大小写字母和数字组成");
        jQuery("#pwdspan").attr("class", "error");
        fn_setpwdsl(0);
        return false;
    }
    var ppattern = /^[0-9A-Za-z]{6,16}$/;
    if (! ppattern.test($("#usrpwd").val())) {
        ShowMsg("pwdspan", 0, "6-16位的字符!由大小写字母和数字组成");
        jQuery("#pwdspan").attr("class", "error");
        fn_setpwdsl(0);
        return false;
...
function ChkEmail() {
    ...
    /* if(mail_str.match(/^[-_.a-z0-9A-Z]+([-_.][a-zA-Z0-9]+)*@([a-zA-Z0-9]+([-_.][a-zA-Z0-9]+))+$/i)){ */
    if (mail_str.match(/^([a-zA-Z0-9_\.\-])+\@(([a-zA-Z0-9\-])+\.)+([a-zA-Z]{2,4})+$/)) {
        ///(^[a-zA-Z0-9_-]{1,})+@([a-zA-Z0-9]{1,})+\.([a-zA-Z]{2,})$/g)){
