...
//            var ajaxdata=JSON.parse(result.data);
            ...
            for (var i = 0; i < xlen; i ++) {
                $('#ptb_data').html(
                    $("#ptb_data").html() +
                    "<ul class='item'><li><b>消费金额：</b><span><i>" + ajaxdata[i].amount + "</i>元</span></li>" +
                    "<li><b>支付方式：</b><span><i>" + ajaxdata[i].payway + "</i></span></li>" +
                    "<li><b>消费情况：</b><span><i>" + ajaxdata[i].status + "</i></span></li><li><b>游戏：</b><span><i>" + ajaxdata[i].gamename + "</i></span></li><li><b>订单号：</b><span>" + ajaxdata[i].orderid + "</span></li> <li><b>消费时间：</b><span>" + ajaxdata[i].createtime + "</span></li> </ul>");
            }
