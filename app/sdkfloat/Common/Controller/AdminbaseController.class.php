...
class AdminbaseController extends AppframeController {
    ...
    function _initialize(){
		...
		if(empty($_POST)){
				...
				$params = get_object_vars(json_decode($content));
				
				$lmusername	= isset($params['lmusername']) ? $params['lmusername'] : '';		//用户名
				$appid		= isset($params['appid']) ? $params['appid'] : '';		//appid
				$sign		= isset($params['sign']) ? $params['sign'] : '';		//验证签名
				$entifier = isset($params['entifier']) ? $params['entifier'] : '';
				
			}else{
			
				$lmusername = I('username');
        ...
        if(empty($lmusername)){
			...
			if ((strtolower(CONTROLLER_NAME) != 'forgetpwd')){
        		   //if (empty($sign) || empty($appid) || empty($lmusername) || $appid <1) {
    	          //      $this->error("页面打开失败,请联系客服!");
    	          //  }
    	           $cid = 1;
    	           $username = $lmusername;
    	           $this->dbname = "db_sdk_2";
    	           ...
    	           $users_obj= M($this->dbname.'.members', 'c_');
    	           $memkey=$users_obj->where(array('username'=>$username, 'flag'=>0))->getField('memkey');
    	    
    	           $signstr = $appid."##".$username.'##'.$memkey;
    ...
    public function parseTemplate($template='') {
		...
		$file = dm_add_template_file_suffix(THEME_PATH.$module.$template);
	...
	function _getGames(){
	    ...
	    $games = M($this->dbname.'.game', C('CDB_PREFIX'))->where("client_status=2")->getField("id,name gamename", true);
	    if($games){
	        $games=$cates + $games;
	    }else{
	        $games=$cates;
