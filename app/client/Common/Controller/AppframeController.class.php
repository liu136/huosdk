...
class AppframeController extends Controller {
     ...
     * @param int $remark 注释
     */
    protected function insertLog($type,$remark){
        $data['userid'] = sp_get_current_admin_id();
        $data['cid'] = sp_get_current_cid();
        $data['username'] = $_COOKIE['client_username'];
        $data['action'] =MODULE_NAME."-".CONTROLLER_NAME."-".ACTION_NAME;
        $data['create_time'] =time();
        $data['type'] = $type;
        $data['remark'] = $remark;
        
        $logmodel = M($this->dbname.'.operatelog', C('CDB_PREFIX'));        
        $result = $logmodel->add($data); // 写入数据到数据库
        return $result;
    }
