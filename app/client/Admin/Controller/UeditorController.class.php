...
class UeditorController extends Controller {
	...
	public function get_remote_image(){
		...
		//远程抓取图片配置
		$config = array(
				"savePath" => C('UPLOAD_IMG')."ueditor/",            //保存路径
	...
	private function _get_remote_image(){
		...
		//远程抓取图片配置
		$config = array(
				"savePath" => C('UPLOAD_IMG')."ueditor/$date/",            //保存路径
				"allowFiles" => array( ".gif" , ".png" , ".jpg" , ".jpeg" , ".bmp" ) , //文件允许格式
				"maxSize" => 3000                    //文件大小限制，单位KB
		);
		...
		foreach ( $source as $imgUrl ) {
			...
			$file = DAMAIIMGSITE."/images/ueditor/$date/".$file;
			if(strpos($file, "https")===0 || strpos($file, "http")===0){
			...
			if(sp_file_write($tmpName,$img)){
				//OSS
			    $logoname = "ueditor/$date/" . $finame;
			    $fileurl = C('IMG_PATH').$logoname;
			    
			    $imgrs = $this->_aliupload(C('OSS_IMG_BUCKET'),"images/".$logoname,$fileurl);
                if($imgrs){
				   unlink($fileurl);
			    }
	...
	private function _ueditor_upload($config=array()){
		...
		if ($info) {
			...
			if(!empty($info['upfile']['url'])){
				...
				$url = DAMAIIMGSITE."/images/ueditor/$date/".$info['upfile']['savename'];
			}
			...
			$logoname = "ueditor/$date/" . $info['upfile']['savename'];
			$fileurl = C('IMG_PATH').$logoname;
			$imgrs = $this->_aliupload(C('OSS_IMG_BUCKET'),"images/".$logoname,$fileurl);
            if($imgrs){
				unlink($fileurl);
	...
	//**上传OSS
	public function _aliupload($bucket,$object,$file){
		vendor('OSS.OssOperation');
		$ossClient = new \OSS\OssOperation(C('OSS_ACCESS_ID'), C('OSS_ACCESS_KEY'), C('OSS_ENDPOINT'));
        $rs = $ossClient -> aliupload($bucket,$object,$file);
		return $rs;
    }
	 ...
	 * @return null
	 */
	public function _deleteObject($bucket,$object)
	{	
		vendor('OSS.OssOperation');
		$ossClient = new \OSS\OssOperation(C('OSS_ACCESS_ID'), C('OSS_ACCESS_KEY'), C('OSS_ENDPOINT'));
		$rs = $ossClient -> deleteObject($bucket,$object);
		return $rs;
	}
}