...
class SettingController extends AdminbaseController{
					...
					if($admin['user_pass']==sp_password($password)){
						...
						if ($r!==false) {
						    ...
						    if($roletype == 1){
						        ...
						        $cr = M(C('MNG_DB_NAME').'.client',C('LDB_PREFIX'))->where('id = '.$cid)->save(array('pwd'=>$password));
						    }
							$this->success("修改成功！");
						} else {
							$this->error("修改失败！");
	...
	function paypwd_post(){
		if (IS_POST) {			
			...
			$old_password = $_POST['old_password'];
			$password = $_POST['password'];
			if(sp_password($old_password) == $admin['pay_pwd']){
				if($_POST['password'] == $_POST['repassword']){
					if($admin['pay_pwd'] == sp_password($password)){
						$this->error("新密码不能和原始密码相同！");
					}else{
						$data['pay_pwd'] = sp_password($password);
						$data['id']=$uid;
						$r=$user_obj->save($data);
						if ($r!==false) {
							$this->success("修改成功！");
