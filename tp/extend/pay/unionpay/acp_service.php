...
class AcpService {
    ...
    public static function updateEncryptCert(&$params) {
        ...
        if ($certType === "01") {
            ...
            if (CertUtil::getEncryptCertId() != $certInfo['serialNumber']) {
                $newFileName = getBackupFileName(SDK_ENC_CERT_PATH);
                // 将原证书备份重命名
                if (!copy(SDK_ENC_CERT_PATH, $newFileName)) {
                    $logger->LogError('原证书备份失败');
                    return -1;
                }
                // 更新证书
                if (!file_put_contents(SDK_ENC_CERT_PATH, $strCert)) {
    ...
    static function sign(&$params) {
        if ($params['signMethod'] == '01') {
            return AcpService::signByCertInfo($params, SDK_SIGN_CERT_PATH, SDK_SIGN_CERT_PWD);
        } else {
    ...
    static function post($params, $url) {
        ...
        ); // http://php.net/manual/en/function.curl-setopt.php页面搜CURL_SSLVERSION_TLSv1
        curl_setopt(
...
eot;
    ...
    static function getCustomerInfoWithEncrypt($customerInfo) {
        ...
        if (count($encryptedInfo) > 0) {
            ...
            $encryptedInfo = AcpService::encryptData($encryptedInfo, SDK_ENC_CERT_PATH);
