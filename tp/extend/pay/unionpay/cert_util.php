...
//         $cert->certId = $certId;
    ...
    {
    	if( $certPath == null ) {
    		$certPath = SDK_SIGN_CERT_PATH;
    		$certPwd = SDK_SIGN_CERT_PWD;
    	}
...
// 	    $cert->certId = $certId;
    ...
    public static function verifyAndGetVerifyCert($certBase64String){
		...
		if (SDK_MIDDLE_CERT_PATH === null || SDK_ROOT_CERT_PATH === null){
		...
		if(strtolower(IF_VALIDATE_CN_NAME) == "true"){
			if (COMPANY != $cn){
		...
		$result = openssl_x509_checkpurpose($certBase64String, X509_PURPOSE_ANY, array(SDK_ROOT_CERT_PATH, SDK_MIDDLE_CERT_PATH));
		if($result === FALSE){
