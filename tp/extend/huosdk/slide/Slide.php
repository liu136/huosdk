...
class Slide {
    ...
    public function getDetail($cid) {
        ...
        $from = input('get.from/d',3);
        $_map['slide_cid'] = $cid;
        $_rdata['count'] = $_slide_model->alias("s")->join(
            Config::get('database.prefix').'game g', 's.target_id=g.id AND g.classify ='.$from
        )->where($_map)->count();
        if ($_rdata['count'] > 0) {
            $_field = [
                'slide_name'                                               => 'name',
            ...
            $_rdata['list'] = $_slide_model->alias("s")->join(
                Config::get('database.prefix').'game g', 's.target_id=g.id AND g.classify ='.$from
            )->field($_field)->where($_map)->select();
//            $_rdata['list'] = $_slide_model->field($_field)->where($_map)->select();
    ...
    public function getList($type = '') {
        ...
        $_map['cat_status'] = ['eq', 1];
        //$_sc_data = Db::name('slide_cat')->where($_map)->select();
        $_cat_id = Db::name('slide_cat')->where($_map)->value('cid');
        $field
            = "slide_name name, type_id type, target_id target,slide_url url, slide_pic image, slide_content content,slide_des `desc`, cid, cat_idname";
        $_sc_data = Db::name('slide')
                      ->alias('s')
                      ->field($field)
                      ->join('__SLIDE_CAT__ c', 'c.cid=s.slide_cid')
                      ->where('slide_cid', $_cat_id)
                      ->select();
        if (empty($_sc_data)) {
            return null;
        }
