...
class HuoSession {
     ...
     * @param $name
     */
    public static function setUser($value, $name = '') {
        $_prefix = 'user';
        if (is_array($value)) {
            foreach ($value as $_k => $_v) {
                Session::set($_k, $_v, $_prefix);
            }
            return;
        }
        Session::set($name, $value, $_prefix);
    }
...
//    }
    ...
    public function setUsertoken($mem_id, $rq_data, $uptoken = true) {
        if ($rq_data['app_id'] != Config::get('config.APP_APPID')) {
            $_uptoken = false;
        } else {
            $_uptoken = $uptoken;
        }
    ...
    public function setSession(array $rq_data) {
        ...
        if (false === $_session_id) {
            ...
            if (empty($rq_data['app_id']) || $rq_data['app_id'] != Config::get('config.APP_APPID')) {
                return session_id();
            } else {
                return false;
            }
