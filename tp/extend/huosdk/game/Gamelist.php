...
class Gamelist {
    ...
    public function _testList(array $game_map, $page, $offset) {
    ...
    public function testList(array $game_map, $page, $offset, $test_type = 0) {
        ...
        if ($test_type > 0) {
            ...
            switch ($test_type) {
                case 1:
                    $_own_map['gt.start_time'] = ['between', [$time1, $time2]];
                    break;
                case 2:
                    $_own_map['gt.start_time'] = ['gt', $time2];
                    break;
                case 3:
                    $_own_map['gt.start_time'] = ['lt', time()];
                    break;
            }
            $_map = array_merge($_own_map, $game_map);
            $_page = $page." , ".$offset;
            ...
            $_own_map['gt.start_time'] = ['between', [$time1, $time2]];
            $_map1 = array_merge($_own_map, $game_map);
            //今日开测
            $_list = Db::name('game_test')
                       ->alias('gt')
                       ->field($_field)
            ...
            //即将开测
            $_own_map['gt.start_time'] = ['gt', $time2];
            $_map2 = array_merge($_own_map, $game_map);
            $_list = Db::name('game_test')
                       ->alias('gt')
                       ->field($_field)
                       ->join($_join)
            ...
            //已经开测
            $_own_map['gt.start_time'] = ['lt', time()];
            $_map3 = array_merge($_own_map, $game_map);
            $_list = Db::name('game_test')
                       ->alias('gt')
                       ->field($_field)
                       ->join($_join)
    ...
    public function _serverList(array $game_map, $page, $offset) {
        $_base_field = $this->getListfield();
        $_own_field = [
            'gs.id'         => 'serid',
            'gs.ser_name'   => 'sername',
    ...
    public function serverList(array $game_map, $page, $offset, $server_type = 0) {
        $_base_field = $this->getListfield();
        $time1 = strtotime(date("Y-m-d").' 00:00:00');
        $time2 = strtotime(date("Y-m-d").' 23:59:59');
        $_own_field = [
        ...
        if ($server_type > 0) {
            ...
            switch ($server_type) {
                case 1:
                    $_own_map['gs.start_time'] = ['between', [$time1, $time2]];
                    break;
                case 2:
                    $_own_map['gs.start_time'] = ['gt', $time2];
                    break;
                case 3:
                    $_own_map['gs.start_time'] = ['lt', time()];
                    break;
            }
            $_map = array_merge($_own_map, $game_map);
            $_page = $page." , ".$offset;
            ...
            //已经开服
            $_own_map['gs.start_time'] = ['lt', $time2];
            $_map3 = array_merge($_own_map, $game_map);
            $_list = Db::name('game_server')
                       ->alias('gs')
                       ->field($_field)
...
//        print_r($_map);exit;
    ...
    private function setAssigninfo(array &$gamelist) {
        ...
        foreach ($gamelist as $_k => $_v) {
            ...
            if ($_v['classify'] == 4) {
                $gamelist[$_k]['downlink'] = $_v['yiosurl'];
            }
            unset($gamelist[$_k]['yiosurl']);
        }
