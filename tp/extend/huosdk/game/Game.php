...
class Game {
    ...
    public function getUpinfo($client_id = 0, $verid = 0) {
        $_client_id = $client_id;
        if (empty($_client_id)) {
            $_client_id = $this->client_id;
        }
    ...
    public function getGamedetail($app_id = 0, $from = 3) {
        ...
        if (3 == $from) {
            ...
            if (!empty($_game_info['yiosurl'])) {
                $_game_info['downlink'] = $_game_info['yiosurl'];
            } else {
        ...
        $_serlist = $_ser_class->getDetailserver($app_id);
    ...
    public function getGameList($where) {
        ...
        /* 是否公益服*/
        if (isset($where['iswelfare']) && 2 == $where['iswelfare']) {
            $_map['g.is_welfare'] = 2;
        }
        /* 搜索游戏名称 */
        if (!empty($where['name'])) {
        ...
        if (isset($where['test']) && 2 == $where['test']) {
            ...
            return $_glist_class->testList($_map, $_page, $_offset, $where['test_type']);
        }
        /* 是否开服 2 开服游戏 1 普通 0 */
        if (isset($where['server']) && 2 == $where['server']) {
            return $_glist_class->serverList($_map, $_page, $_offset, $where['server_type']);
        }
        /* 是否推荐 2 推荐 1 普通 0 所有 */
        ...
        if (!empty($where['type'])) {
            return $_glist_class->typeList($_map, $_page, $_offset, $where['type']);
        }
        /* 是否随机推荐 */
        $_rand_flag = false;
    ...
    public function game_contact() {
        $_map['app_id'] = $this->app_id ? $this->app_id : 0;
//        $_map['app_id'] = 0;
        $_hinfo = Db::name('game_contact')->where($_map)->find();
        if (!empty($_hinfo)) {
            $_hinfo['qq'] = explode(',', $_hinfo['qq']);
            $_hinfo['qqgroup'] = explode(',', $_hinfo['qqgroup']);
            $_hinfo['qqgroupkey'] = explode(',', $_hinfo['qqgroupkey']);
        }
