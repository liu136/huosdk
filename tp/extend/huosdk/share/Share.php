...
class Share {
    ...
    public function getGame($mem_id = 0, $game_id = 0) {
        $_app_id = $game_id;
        $_mem_id = $mem_id;
        $cfg_app_congig = Config::get('config.HUOAPP');
        if (empty($_mem_id)) {
            return false;
        }
//        $_url = Url::build('wap/Game/read', ['gameid' => $_app_id, 'agentid' => $_mem_agent_id]);
//        $_url = Config::get('domain.SDKSITE').$_url;
//        if (in_array($_app_id, $cfg_app_congig)) { //没考虑双端问题
        if ($_app_id == $cfg_app_congig['APP_APPID']) {//安卓
            //分享app, 则分享每个app的下载地址
            $_gp_class = new Gamepack($mem_id);
            $_url = $_gp_class->getMemAppurl($mem_id);
        } elseif ($_app_id == $cfg_app_congig['IOS_APP_APPID']) {//ios
            $_url = "http://api.niudaosy.com/ios/index.html";
        } else {
            //分享游戏 则分享官方游戏
            $_g_class = new  Game();
        ...
        $_rdata['title'] = $_title ? $_title : '光奈网络sdk';
        $_rdata['url'] = $_url;
        $_rdata['sharetext'] = $_content ? $_content : '光奈网络sdk下载地址';
