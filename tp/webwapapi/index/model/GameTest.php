...
class GameTest extends Model {
    ...
    public function getGameTestServers() {
        ...
        $server_lists = $this->indexGameTestServers($where);

        $end_time = strtotime(date('Y-m-d', time()))+86400;
        $where['s.start_time'] = array('GT', $end_time);

        $about_lists = $this->indexGameTestServers($where);

        if(count($server_lists) > 0){
            $server_lists = array_merge($server_lists,$about_lists);
        }else{
            $server_lists = $about_lists;
        }

        $start_time = strtotime(date('Y-m-d', time()));
        $where['s.start_time'] = array('LT', $start_time);

        $after_lists = $this->indexGameTestServers($where);

        if(count($server_lists) > 0){
            $server_lists = array_merge($server_lists,$after_lists);
        }else{
            $server_lists = $after_lists;
        }
    ...
    public function indexGameTestServers($where, $limit=30) {
        $where['s.is_delete'] = 2;
        $where['g.is_app'] = 2;
        $field = "s.start_time, g.name, s.status, g.id app_id";
        $field .= ", g.icon game_icon, testdesc server_name";
