...
class Gamemoney extends Basehuo {
    ...
    function index() {
        ...
        if (empty($_game_id)) {
            return hs_huosdk_responce('400', '游戏参数错误');
        }
        Session::set('game_id', $_game_id, 'app'); /* 充值游戏币 游戏 */
        if (empty($_money)) {
            return hs_huosdk_responce('400', '金额参数错误');
        }
        /* 获取我的代金卷 */
        ...
        if (false == $_remain_money) {
            return hs_huosdk_responce(400, '代金卷错误');
        }
    ...
    public function preorder() {
        ...
        return hs_huosdk_responce('200', '获取成功', $_rdata, $this->auth_key);
    }
    ...
    public function pay() {
        ...
        if ($_paytoken != $_s_paytoken) {
            return hs_huosdk_responce(400, "非法请求,参数错误");
        }
        if ($_order_id != Session::get('order_id', 'order')) {
            return hs_huosdk_responce(400, "订单已失效，请重新下单");
        }
        $_time = time();
        $_order_time = Session::get('order_time', 'order');
        /* 86400s 不支付 订单失效 */
        if ($_order_time + 86400 < $_time) {
            return hs_huosdk_responce(400, "订单已失效,请重新选择支付");
        }
        $_pay_class = Wallet::init();
        $_rs = $_pay_class->upPayway($_order_id, $_payway);
        if (!$_rs) {
            return hs_huosdk_responce(1000, "订单不存在");
        }
        /* 通过支付方式下单 */
        $_pay_class = \huosdk\pay\Driver::init($_payway);
        $_payinfo = $_pay_class->clientPay();
        if (empty($_payinfo)) {
            return hs_huosdk_responce(400, "下单失败");
        }

        return hs_huosdk_responce(200, "请求成功", $_payinfo, $this->auth_key);
    }
}