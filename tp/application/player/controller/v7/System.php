...
use huosdk\game\Notice;
use huosdk\game\Version;
use huosdk\game\Update;
...
//            $this->_error("The file {$file} is not exists");
    ...
    function open() {
        $_key_arr = array(
            'app_id', 'client_id', 'from', 'device_id', 'userua', 'open_cnt','version'
        );
        ...
        $version = $this->getVal($_data, 'version', 0);
        ...
        $is_ios = $this->getVal($_data, 'is_ios', 0);
        ...
        $_rdata['agent_id'] =$_gol_data['agent_id'];
        ...
        $_gv_update=new Update();
        $_rdata['up_status'] = $_gv_update->getupdatestatus($_gol_data['app_id'], $_gol_data['agent_id'],$version);//0 关闭 1普通更新 2强制更新 
        $_rdata['up_url'] = $_gv_update->getupdateurl($_gol_data['app_id'], $_gol_data['agent_id'],$is_ios);
        $_rdata['up_content'] = $_gv_update->getupdateinfo($_gol_data['app_id']);
        ...
        //公告
        // $_rdata['notice_content'] = 'sdk测试公告,请勿点击';   
        // $_rdata['notice_status'] = '1'; 
...
//        $this->setStartsession();
        ...
        $_rdata['notice_list'] = $_notice_class->getNotice($_gol_data['app_id']);
        Session::set('open_cnt', $this->rq_data['open_cnt'], 'device');
        //获取第三方登陆参数
//        $_field = [
//            "IFNULL(oauth_appid,'')"       => 'oauth_appid',
//            "IFNULL(oauth_appsecret,'')"   => 'oauth_appsecret',
//            "IFNULL(oauth_redirecturl,'')" => 'oauth_redirecturl',
//            "oauth_type"                   => 'oauth_redirecturl'
//        ];
        $condition = [
            'app_id' => $_gol_data['app_id'],
            'status' => 1,
        ];
        $_rdata['oauth_info'] = Db::name("game_oauth")->where($condition)->select();//->field($_field)
        /*增加浮点显示控制 1为不显示，2为显示 2017.8.1 ou start*/
        $_f_class = new \huosdk\floater\Floater($_gol_data['app_id']);
        $_rdata['float_is_show'] = $_f_class->getFloatstatus($_gol_data['app_id'], $version);
        /*增加浮点显示控制 end*/
        return hs_player_responce(200, 'INIT OK!'.$_rdata['agent_id'], $_rdata, $this->auth_key);
    }
