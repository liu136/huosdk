...
class Usermsg extends Baseplayer {
    ...
    public function index() {
        ...
        if (empty($_rdata['count'])) {
            return hs_huosdk_responce('200', '无消息');
        }
    ...
    public function delete() {
        ...
        if (empty($_msg_id)) {
            return hs_huosdk_responce('400', '未找到消息');
        }
        $_msg_class = new Memmsg($this->mem_id);
        $_rs = $_msg_class->delete($_msg_id);
        if (false == $_rs) {
            return hs_huosdk_responce('400', '删除失败');
        }
        return hs_huosdk_responce('200', '删除成功');
    }
}