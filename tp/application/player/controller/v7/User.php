...
class User extends Basehuo {
    ...
    protected function getReturn($mem_info, $agentgame = '', $flag = 0) {
        ...
        //登陆处理
        $_login_data['mem_id'] = Session::get('id', 'user');  //存在游戏账号取游戏账号，默认没有
        ...
        $_rdata['mem_id'] = empty($mem_info['gameaccount'])?$_login_data['mem_id']:$mem_info['gameaccount'];  //存在游戏账号取游戏账号，默认没有
    ...
    public function redinfo() {
        ...
        if (empty($_mem_info)) {
            return hs_huosdk_responce('300', '获取信息失败');
        } else {
        ...
        return hs_huosdk_responce('200', '请求成功', $_mem_info, $this->auth_key);
    }
    ...
    public function setpwd() {
        ...
        if (empty($_mobile) && empty($_email)) {
            return hs_huosdk_responce(400, '参数错误');
        ...
        if (false == $_rs) {
            return hs_huosdk_responce(1000, '修改失败');
        }

        return hs_huosdk_responce(200, '修改成功', ['status' => 2], $this->auth_key);
    }
    ...
    public function remove_bindmobile() {
        ...
        if (false == $_rs) {
            return hs_huosdk_responce(400, '您已经解除绑定了');
        }
        $_rdata['status'] = 2;

        return hs_huosdk_responce(200, '解除绑定成功', $_rdata, $this->auth_key);
    }
...
//        $this->isUserLogin();
        ...
        if (empty($_username)) {
            ...
            if (empty($_username)) {
                ...
                if (empty($_username)) {
                    return hs_huosdk_responce(413, '邮箱错误');
                }
            }
        }
        ...
        if (200 == $_rdata['code']) {
            ...
            return hs_huosdk_responce(200, $_rdata['msg'], $_rd, $this->auth_key);
        } else {
            // 邮件发送失败
            return hs_huosdk_responce($_rdata['code'], $_rdata['msg']);
        }
    }
    ...
    public function checkEmailCode($email = '', $code = '') {
        ...
        if ($_rs['code'] != 200) {
            return hs_huosdk_responce($_rs['code'], $_rs['msg']);
        }
    ...
    public function bindemail() {
        ...
        if (false == $_rs) {
            return hs_huosdk_responce(400, '请勿重复绑定');
        }
        $_rdata['status'] = 2;

        return hs_huosdk_responce(200, '绑定成功', $_rdata, $this->auth_key);
    }
    ...
    public function remove_bindemail() {
        ...
        if (false == $_rs) {
            return hs_huosdk_responce(400, '您已解绑');
        }
        $_rdata['status'] = 2;
    ...
    public function setintroducer() {
        ...
        if (401 == $_rs) {
            return hs_huosdk_responce(401, '请输入正确的邀请人');
        }
        if (402 == $_rs) {
            return hs_huosdk_responce(402, '已经绑定过邀请人了');
        }
        if (403 == $_rs) {
            return hs_huosdk_responce(403, '邀请人不存在');
        }
        if (404 == $_rs) {
            return hs_huosdk_responce(404, '系统内部错误');
        }
