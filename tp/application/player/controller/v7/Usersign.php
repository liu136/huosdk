...
class Usersign extends Baseplayer {
    ...
    public function index() {
        ...
        if (empty($_rdata)) {
            return hs_huosdk_responce('400', '请求失败');
        }
    ...
    public function save() {
        ...
        if (false === $_itg) {
            return hs_huosdk_responce('400', '签到失败');
        }
        ...
        if (-1 === $_itg) {
            $_rdata['status'] = 3;
            return hs_huosdk_responce('200', '今日已签到');
        }

        $_rdata['status'] = 2;
        return hs_huosdk_responce('200', '签到成功', $_rdata, $this->auth_key);
    }
}