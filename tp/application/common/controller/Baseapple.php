...
class Baseapple extends Base {
    ...
    public function getParam() {
        $token= isset($_POST['uptoken'])?$_POST['uptoken']:(isset($_GET['uptoken'])?$_GET['uptoken']:"");
        $_url_param = $this->request->param();
        if(Cache::has($token)){
            $_rq_data= Cache::get($token."_rq_data") ;
             
        }else if( isset($_url_param['h5_data'])){
            $h5_data=base64_decode($_url_param['h5_data']);
            $_rq_data = json_decode($h5_data, true);
     ...
     * 返回信息处理
     *
     * @param int    $check      是否切换用户    0 不更换 1 更换支付 2 重新初始化
     * @param string $user_token 玩家此次连接token
     * @param string $agent_game 所属渠道    玩家游戏渠道编号
     * @param string $Code_str   混淆代码的前缀
     *
     * @return $this
     */
    public function initReturn($check = 0, $user_token = '', $agent_game = '',$Code_str='') {
        $_rdata[$Code_str.'cc'] = $check;     //是否切换用户
        $_rdata[$Code_str.'ut'] = $user_token;
        $_rdata[$Code_str.'ag'] = $agent_game;
     ...
     * @return $this
     */
    public function rgReturn($mem_id = 0, $cp_user_token = '', $agent_game = '', $float_is_show = 1) {
        $_rdata['mid'] = $mem_id;     //是否切换用户
        $_rdata['cptoken'] = $cp_user_token;
        $_rdata['ag'] = $agent_game;
        $_rdata['sh'] = $float_is_show;
        return hs_api_responce(200, '注册成功', $_rdata);
    }
