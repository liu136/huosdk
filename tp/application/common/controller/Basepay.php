...
class Basepay extends Base {
    ...
    protected function _initialize() {
        ...
        $_userid= isset($_POST['userid'])?$_POST['userid']:(isset($_GET['userid'])?$_GET['userid']:"");
        if(!empty($_userid) && Cache::has($_userid)){
            $_auth_key=Cache::get($_userid."_key") ;
            $this->auth_key =$_auth_key ;
            $this->web_key = Cache::get($_userid."_key") ;
            $this->rq_data= Cache::get($_userid."_rq_data") ;
            ...
            $this->setFloatid();
            return ;
         }else if (!empty($_key) && !empty($_data)) {
            // 解析请求数据
