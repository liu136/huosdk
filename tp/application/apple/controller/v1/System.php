...
class System extends Baseapple {
    ...
    function open() {
        $_key_arr = array(
            'app_id', 'client_id', 'from', 'device_id', 'userua'
        );
        ...
        if($_client_key != $akd){
            return hs_player_responce('1001', '初始化失败，参数错误！');
        }

        $_Code_class = new \huosdk\pay\Pay();
        $FunctionStr = $_Code_class->getPayFunctionStr($_gol_data['app_id']);
        $start_str="";
        if( $FunctionStr){
             $start_str=$FunctionStr['code_str_start']; //混淆代码的前缀
        }
       
        return $this->initReturn(0, $_user_token, $_agentgame,$start_str);
    }
}