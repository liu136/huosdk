...
class GameController extends MobilebaseController {
     ...
     */
    public function app() {
        $_app_map['app_id'] = C('IOSAPP_APPID');
        $_app_map['status'] = 2;
        $_app_ver_data = M('game_version')->where($_app_map)->order('id desc')->find();
        $ios_app_down_url = DOWNIOSSITE.$_app_ver_data['packageurl'];
        $mobileprovision_url = APKSITE.'/embedded.mobileprovision';
        $this->assign('ios_app_down_url', $ios_app_down_url);
        $this->assign('mobileprovision_url', $mobileprovision_url);
        $this->display();
    }

    public function apptrust() {
        $mobileprovision_url = APKSITE.'/embedded.mobileprovision';
        $this->assign('mobileprovision_url', $mobileprovision_url);
        $_device = I('device/d', 3);
        if (3 == $_device){
            $this->display('Game/app_trust_9');
        }else{
            $this->display('Game/apptrust');
        }
