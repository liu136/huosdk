    ...
    public function Pager($Page_tpl = '') {
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            if ($i == $cfg ['pageindex']) {
				...
				$pList .= '<a href="javascript:void(0)" class="pagenum currpage">' . str_replace('*', $i, $cfg ['list']) . '</a> ';
            } else {
                ...
                if ($this->Static && $i == 1) {
                    $pList .= $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '" class="pagenum"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                } else {
                    $pList .= $this->linkwraper_pre.'<a href="' . str_replace('*', $i, $cfg ['link']) . '" class="pagenum"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                }
            }
        }
       ...
       if ($cfg ['pageindex'] > 1) {
            ...
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '" class="prev nojump">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '" class="prev">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
            }
        }else{
			$pPrev = $this->linkwraper_pre.'<a href="javascript:void(0)" class="prev nojump">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
		}
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            $pNext = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '" class="next">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }else{
			$pNext = $this->linkwraper_pre.'<a href="javascript:void(0)" class="next nojump">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
		}
