...
class pmcontrol extends base {
    ...
    function oncheck_newpm() {
        ...
        if ($more == 0) {
            ...
            if ($more == 2) {
                ...
                return array('newpm'       => $newpm, 'newprivatepm' => $newprvpm, 'newchatpm' => $newchatpm,
                             'lastdate'    => $lastpm['lastdateline'], 'lastmsgfromid' => $lastpm['lastauthorid'],
                             'lastmsgfrom' => $lastpm['lastauthorusername'], 'lastmsg' => $lastpm['lastsummary']);
            }
        } elseif ($more == 4) {
            return array('newpm' => $newpm, 'newprivatepm' => $newprvpm, 'newchatpm' => $newchatpm);
        } else {
    ...
    function onsendpm() {
        ...
        if ($replypmid) {
            ...
            $lastpmid = $_ENV['pm']->sendpm(
                $this->user['uid'], $this->user['username'], $msgto, $subject, $message, $type
            );
        }
        return $lastpmid;
    }
    ...
    function onreadstatus() {
        ...
        $this->user['uid'] = intval($this->input('uid'));
        $_ENV['pm']->setpmstatus(
            $this->user['uid'], $this->input('uids'), $this->input('plids'), $this->input('status')
        );
    }
