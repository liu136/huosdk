...
class notemodel {
    ...
    function add($operation, $getdata = '', $postdata = '', $appids = array(), $pri = 0) {
        ...
        $this->db->query(
            ...
            ."notelist SET getdata='$getdata', operation='$operation', pri='$pri', postdata='$postdata'$extra"
        );
        $insert_id = $this->db->insert_id();
        $insert_id
        && $this->db->query(
            "REPLACE INTO ".UC_DBTABLEPRE."vars (name, value) VALUES ('noteexists', '1')$varextra"
        );
        return $insert_id;
    }
    ...
    function sendone($appid, $noteid = 0, $note = '') {
        ...
        if ($app['extra']['apppath'] && @include $app['extra']['apppath'].'./api/'.$apifilename) {
            ...
            $response = trim(
                $_ENV['misc']->dfopen2($url, 0, $note['postdata'], '', 1, $app['ip'], UC_NOTE_TIMEOUT, true)
            );
        }
        ...
        if ($returnsucceed) {
            ...
            $this->db->query(
                "UPDATE ".UC_DBTABLEPRE
                ."notelist SET app$appid='1', totalnum=totalnum+1, succeednum=succeednum+1, dateline='{$this->base->time}' $closedsqladd WHERE noteid='$note[noteid]'",
                'SILENT'
            );
            $return = true;
        } else {
            $this->db->query(
                "UPDATE ".UC_DBTABLEPRE
                ."notelist SET app$appid = app$appid-'1', totalnum=totalnum+1, dateline='{$this->base->time}' $closedsqladd WHERE noteid='$note[noteid]'",
                'SILENT'
            );
            $return = false;
        }
    ...
    function _get_note() {
        ...
        $data = $this->db->fetch_first(
            "SELECT * FROM ".UC_DBTABLEPRE."notelist WHERE closed='0' AND $app_field<'1' AND $app_field>'-"
            .UC_NOTE_REPEAT."' LIMIT 1"
        );
        return $data;
    }
    ...
    function get_url_code($operation, $getdata, $appid) {
        ...
        $code = urlencode(
            $this->base->authcode(
                "$action&".($getdata ? "$getdata&" : '')."time=".$this->base->time, 'ENCODE', $authkey
            )
        );
