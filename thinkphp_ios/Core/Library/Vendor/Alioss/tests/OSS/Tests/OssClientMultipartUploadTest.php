...
class OssClientMultipartUploadTest extends TestOssClientBase {
    public function testInvalidDir() {
        try {
            ...
            $this->assertEquals(
                "parameter error: abc/ds/s/s/notexitst is not a directory, please check it", $e->getMessage()
            );
        }
    }
    ...
    public function testMultipartUploadBigFile() {
        ...
        try {
            $this->ossClient->multiuploadFile(
                $this->bucket, $object, $bigFileName, array(OssClient::OSS_PART_SIZE => 1)
            );
    ...
    public function testCopyPart() {
         ...
         */
        $copyId = 1;
        $eTag = $this->ossClient->uploadPartCopy(
            $this->bucket, $copiedObject, $this->bucket, $object, $copyId, $upload_id
        );
        $upload_parts[] = array(
            'PartNumber' => $copyId,
            'ETag'       => $eTag,
    ...
    function testPutObjectsByDir() {
        ...
        sleep(1);
        $this->assertTrue(
            $this->ossClient->doesObjectExist($this->bucket, 'samples/codes/'."OssClientMultipartUploadTest.php")
        );
    }
