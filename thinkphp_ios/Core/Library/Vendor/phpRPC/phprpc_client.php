...
class _PHPRPC_Client {
    ...
    function useService($serverURL, $username = null, $password = null) {
        ...
        if (!isset($urlparts['path'])) {
            ...
            $urlparts['path'] = substr($_SERVER["PHP_SELF"], 0, strrpos($_SERVER["PHP_SELF"], '/') + 1)
                                .$urlparts['path'];
        }
    ...
    function _post($request_body) {
        ...
        if (is_null($this->_proxy)) {
            ...
            $url
                = "{$this->_server['scheme']}://{$this->_server['host']}:{$this->_server['port']}{$this->_server['path']}";
            $connection = "Proxy-Connection: ".($this->_keep_alive ? 'keep-alive' : 'close')."\r\n";
            if (!is_null($this->_proxy['user'])) {
                $connection .= "Proxy-Authorization: Basic ".base64_encode(
                        $this->_proxy['user'].":".$this->_proxy['pass']
                    )."\r\n";
            }
        }
        $auth = '';
        ...
        $response_body = '';
        if (isset($response_header['transfer_encoding'])
            && (strtolower($response_header['transfer_encoding']) == 'chunked')
        ) {
        ...
        if (isset($response_header['content_encoding'])
            && (strtolower($response_header['content_encoding']) == 'gzip')
        ) {
