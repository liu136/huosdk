...
class RongCloud {
    ...
    public function getToken($userId, $name, $portraitUri) {
        try {
            ...
            $ret = $this->curl(
                '/user/getToken', array('userId' => $userId, 'name' => $name, 'portraitUri' => $portraitUri)
            );
            if (empty($ret)) {
    ...
    public function messagePublish(
        $fromUserId, $toUserId = array(), $objectName, $content, $pushContent = '', $pushData = ''
    ) {
        try {
            if (empty($fromUserId)) {
    ...
    public function messageGroupPublish(
        $fromUserId, $toGroupId = array(), $objectName, $content, $pushContent = '', $pushData = ''
    ) {
        try {
            if (empty($fromUserId)) {
                throw new Exception('发送人用户 Id 不能为空');
     ...
     * @return json|xml
     */
    public function messageDiscussionPublish(
        $fromUserId, $toDiscussionId, $objectName, $content, $pushContent = '', $pushData = ''
    ) {
        try {
            if (empty($fromUserId)) {
                throw new Exception('发送人用户 Id 不能为空');
