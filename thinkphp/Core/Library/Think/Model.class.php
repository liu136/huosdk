...
class Model {
    ...
    // 链操作方法列表
    protected $methods
        = array('strict', 'order', 'alias', 'having', 'group', 'lock', 'distinct', 'auto', 'filter', 'validate',
    ...
    public function selectAdd($fields = '', $table = '', $options = array()) {
        ...
        // 写入数据到数据库
        if (false ===
            $result = $this->db->selectInsert($fields ?: $options['field'], $table ?: $this->getTableName(), $options)
        ) {
            // 数据库插入操作失败
    ...
    protected function parseSql($sql, $parse) {
        ...
        if (true === $parse) {
            ...
            $sql = preg_replace_callback(
                "/__([A-Z0-9_-]+)__/sU", function ($match) use ($prefix) {
