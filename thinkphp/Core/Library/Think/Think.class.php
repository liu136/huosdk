...
class Think {
    ...
    static public function start() {
        ...
        if (!APP_DEBUG && Storage::has($runtimefile)) {
            ...
            if (!APP_DEBUG) {
                ...
                $content .= "\nL(".var_export(L(), true).");\nC(".var_export(C(), true).');Think\Hook::import('
                            .var_export(Hook::get(), true).');}';
    ...
    static public function halt($error) {
        ...
        if (APP_DEBUG || IS_CLI) {
            ...
            if (IS_CLI) {
                exit(iconv('UTF-8', 'gbk', $e['message']).PHP_EOL.'FILE: '.$e['file'].'('.$e['line'].')'.PHP_EOL
                     .$e['trace']);
            }
        } else {
            //致命错误信息写入文件2017-09-20添加
            chmod(ERROR_PATH,0777);
            $error_str='['.date('Y-m-d H:i:s').'] ERROR '.$error['message'].' FILE: '.$error['file'].' LINE: '.$error['line'].PHP_EOL;
            file_put_contents(ERROR_PATH.date('Y-m-d').'.log',$error_str,FILE_APPEND);
            //否则定向到错误页面
