...
class BaiduBCS {
    ...
    private function authenticate($opt) {
        ...
        if (!('/' == $opt [self::OBJECT] && '' == $opt [self::BUCKET] && 'GET' == $opt [self::METHOD]
              && !isset ($opt [self::QUERY_STRING] [self::ACL]))
            && !self::validate_bucket($opt [self::BUCKET])
        ) {
    ...
    public function list_bucket($opt = array()) {
        ...
        $this->log(
            $response->isOK() ? "List bucket success!" : "List bucket failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function create_bucket($bucket, $acl = null, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Create bucket success!" : "Create bucket failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function delete_bucket($bucket, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Delete bucket success!" : "Delete bucket failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function set_bucket_acl($bucket, $acl, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Set bucket acl success!" : "Set bucket acl failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function get_bucket_acl($bucket, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Get bucket acl success!" : "Get bucket acl failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function list_object($bucket, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "List object success!" : "Lit object failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function create_object($bucket, $object, $file, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Create object[$object] file[$file] success!"
                : "Create object[$object] file[$file] failed! Response: [".$response->body."] Logid["
                  .$response->header ["x-bs-request-id"]."]", $opt
        );
        return $response;
    }
    ...
    public function create_object_by_content($bucket, $object, $content, $opt = array()) {
        ...
        $this->log(
            $response->isOK()
                ? "Create object[$object] success!" : "Create object[$object] failed! Response: [".$response->body
                                                      ."] Logid[".$response->header ["x-bs-request-id"]."]", $opt
        );
        return $response;
    }
    ...
    public function create_object_superfile($bucket, $object, $file, $opt = array()) {
        ...
        $this->log(
            "File[".$opt ['fileUpload']
            ."], size=[$fileSize], sub_object_size=[$sub_object_size], sub_object_num=[$sliceNum]", $opt
        );
        $object_list = array(
            'object_list' => array());
        for ($i = 0; $i < $sliceNum; $i++) {
            ...
            if (($i + 1) === $sliceNum) {
                //last sub object
                $opt ['length'] = (0 === $fileSize % $sub_object_size) ? $sub_object_size
                    : $fileSize % $sub_object_size;
            } else {
            ...
            $this->log(
                "Begin to upload Sub-object[".$opt [self::OBJECT]."][$i/$sliceNum][seekto:".$opt ['seekTo']."][Length:"
                .$opt ['length']."]", $opt
            );
            $response = $this->create_object($bucket, $opt [self::OBJECT], $file, $opt);
            if ($response->isOK()) {
                $this->log(
                    "Sub-object upload[".$opt [self::OBJECT]."][$i/$sliceNum][seekto:".$opt ['seekTo']."][Length:"
                    .$opt ['length']."]success! ", $opt
                );
        ...
        $this->log(
            $response->isOK()
                ? "Create object-superfile success!" : "Create object-superfile failed! Response: [".$response->body
                                                       ."]", $opt
        );
        return $response;
    }
    ...
    public function upload_directory($bucket, $dir, $opt = array()) {
            ...
            if (isset ($objects [$opt ["seek_object"]])) {
                foreach ($objects as $object => $file) {
                    if ($object != $opt ["seek_object"]) {
                        //当非断点文件，该object已完成上传
                        $this->log(
                            "Seeking[".$opt ["seek_object"]."]. Skip id[$num/$objectCount]object[$object]file[$file].",
                            $opt
                        );
                        ...
                        //当非断点文件，该object已完成上传
                        $this->log(
                            "Found seek id[$num/$objectCount]object[$object]file[$file], begin from here.", $opt
                        );
                        break;
                    }
                    $num++;
            ...
            if (is_int($opt ["seek_object_id"]) && $opt ["seek_object_id"] <= $objectCount) {
                foreach ($objects as $object => $file) {
                    if ($num < $opt ["seek_object_id"]) {
                        $this->log(
                            "Seeking object of [".$opt ["seek_object_id"]
                            ."/$objectCount]. Skip  id[$num/$objectCount]object[$object]file[$file].", $opt
                        );
                        //$result ['skipped'] [] = "[$num/$objectCount]  " . $file;
                        $result ['skipped']++;
                        unset ($objects [$object]);
        ...
        foreach ($objects as $object => $file) {
            ...
            if (isset ($opt [self::IMPORT_BCS_PRE_FILTER]) && function_exists($opt [self::IMPORT_BCS_PRE_FILTER])) {
                ...
                if ($bolRes !== true) {
                    $this->log(
                        "User pre_filter_function return un-true. Skip id[$num/$objectCount]object[$object]file[$file].",
                        $opt
                    );
    ...
    public function copy_object($source, $dest, $opt = array()) {
        ...
        if (empty ($source) || !is_array($source) || !isset ($source [self::BUCKET])
            || !isset ($source [self::OBJECT])
        ) {
            throw new BCS_Exception ('$source invalid, please check!', -1);
        }
        if (empty ($dest) || !is_array($dest) || !isset ($dest [self::BUCKET]) || !isset ($dest [self::OBJECT])
            || !self::validate_bucket($dest [self::BUCKET])
            || !self::validate_object($dest [self::OBJECT])
        ) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Copy object success!" : "Copy object failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function set_object_meta($bucket, $object, $meta, $opt = array()) {
        ...
        $this->log(
            $response->isOK() ? "Set object meta success!" : "Set object meta failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function get_object_acl($bucket, $object, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Get object acl success!" : "Get object acl failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function set_object_acl($bucket, $object, $acl, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Set object acl success!" : "Set object acl failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function delete_object($bucket, $object, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Delete object success!" : "Delete object failed! Response: [".$response->body."]", $opt
        );
        return $response;
    }
    ...
    public function get_object_info($bucket, $object, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Get object info success!" : "Get object info failed! Response: [".$response->body."]",
            $opt
        );
        return $response;
    }
    ...
    public function get_object($bucket, $object, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Get object success!" : "Get object failed! Response: [".$response->body."]", $opt
        );
    ...
    private function check_user_acl($acl) {
        ...
        foreach ($acl ['statements'] as $key => $statement) {
            ...
            if (!isset ($statement ['user']) || !isset ($statement ['resource']) || !isset ($statement ['action'])
                || !isset ($statement ['effect'])
            ) {
            ...
            if (!is_array($statement ['action'])
                || !count(array_diff($statement ['action'], self::$ACL_ACTIONS)) == 0
            ) {
