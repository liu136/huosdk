...
class Oracle extends Driver {
    ...
    protected function parseDsn($config) {
        $dsn = 'oci:dbname=//'.$config['hostname'].($config['hostport'] ? ':'.$config['hostport'] : '').'/'
               .$config['database'];
    ...
    public function execute($str, $fetchSql = false) {
        ...
        if (preg_match("/^\s*(INSERT\s+INTO)\s+(\w+)\s+/i", $str, $match)) {
            ...
            $flag = (boolean)$this->query(
                "SELECT * FROM user_sequences WHERE sequence_name='".strtoupper($this->table)."'"
            );
        }
        //释放前次的查询结果
    ...
    public function getFields($tableName) {
        ...
        $result = $this->query(
            ...
            ."where c.constraint_name=col.constraint_name and c.constraint_type='P'and c.table_name='".strtoupper(
                $tableName
            )
