    ...
    public function entry($id = '') {
        ...
        if ($this->useZh) { // 中文验证码
            for ($i = 0; $i < $this->length; $i++) {
                $code[$i] = iconv_substr(
                    $this->zhSet, floor(mt_rand(0, mb_strlen($this->zhSet, 'utf-8') - 1)), 1, 'utf-8'
                );
                imagettftext(
                    $this->_image, $this->fontSize, mt_rand(-40, 40), $this->fontSize * ($i + 1) * 1.5,
            ...
            for ($i = 0; $i < $this->length; $i++) {
                ...
                imagettftext(
                    $this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.6, $this->_color,
                    $this->fontttf, $code[$i]
                );
            }
        }
    ...
    private function _writeCurve() {
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if ($w != 0) {
                ...
                while ($i > 0) {
                    imagesetpixel(
                        $this->_image, $px + $i, $py + $i, $this->_color
    ...
    private function _writeNoise() {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring(
                    $this->_image, 5, mt_rand(-10, $this->imageW), mt_rand(-10, $this->imageH),
