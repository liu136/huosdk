...
// --------------------------------------------------------------------------------
    ...
    function privParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options = false) {
        ...
        while ($i < $p_size) {
            ...
            if (!isset($v_requested_options[$p_options_list[$i]])) {
                // ----- Error log
                PclZip::privErrorLog(
                    PCLZIP_ERR_INVALID_PARAMETER, "Invalid optional parameter '".$p_options_list[$i]."' for this method"
                );
                // ----- Return
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, PclZip::errorCode(), PclZip::errorInfo());
            ...
            switch ($p_options_list[$i]) {
                ...
                case PCLZIP_OPT_BY_NAME :
                    ...
                    if (is_string($p_options_list[$i + 1])) {
                        ...
                        // ----- Error log
                        PclZip::privErrorLog(
                            PCLZIP_ERR_INVALID_OPTION_VALUE, "Wrong parameter value for option '".PclZipUtilOptionText(
                                                               $p_options_list[$i]
                                                           )."'"
                        );
                        // ----- Return
        ...
        */
                    ...
                    if (!function_exists($v_function_name)) {
                        // ----- Error log
                        PclZip::privErrorLog(
                            PCLZIP_ERR_INVALID_OPTION_VALUE, "Function '".$v_function_name
    ...
    function privAdd($p_filedescr_list, &$p_result_list, &$p_options) {
        ...
        if (($v_zip_temp_fd = @fopen($v_zip_temp_name, 'wb')) == 0) {
            ...
            $this->privSwapBackMagicQuotes();
            PclZip::privErrorLog(
                PCLZIP_ERR_READ_OPEN_FAIL, 'Unable to open temporary file \''.$v_zip_temp_name.'\' in binary write mode'
            );
            // ----- Return
            //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, PclZip::errorCode(), PclZip::errorInfo());
            return PclZip::errorCode();
        ...
        if (($v_result = $this->privWriteCentralHeader(
                $v_count + $v_central_dir['entries'], $v_size, $v_offset, $v_comment
            )) != 1
        ) {
            // ----- Reset the file list
            unset($v_header_list);
    ...
    function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options) {
        ...
        for ($j = 0; ($j < sizeof($p_filedescr_list)) && ($v_result == 1); $j++) {
            ...
            if (($p_filedescr_list[$j]['type'] != 'virtual_file')
                ...
                PclZip::privErrorLog(
                    PCLZIP_ERR_MISSING_FILE, "File '".$p_filedescr_list[$j]['filename']."' does not exists"
                );
                //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, PclZip::errorCode(), PclZip::errorInfo());
                return PclZip::errorCode();
            }
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if (($v_result = $this->privDirCheck(
                        $v_dir_to_check, (($p_entry['external'] & 0x00000010) == 0x00000010)
                    )) != 1
                ) {
...
//      if ($p_entry['compressed_size'] == $p_entry['size'])
    ...
    function privDeleteByRule(&$p_result_list, &$p_options) {
        ...
        if ($v_nb_extracted > 0) {
            ...
            if (($v_result = $v_temp_zip->privWriteCentralHeader(
                    sizeof($v_header_list), $v_size, $v_offset, $v_comment
                )) != 1
            ) {
                // ----- Reset the file list
                unset($v_header_list);
    ...
    function privDecrypt($p_encryption_header, &$p_buffer, $p_size, $p_crc) {
        ...
        $p_buffer = PclZipUtilZipDecrypt(
            $p_buffer, $p_size, $p_encryption_header,
            $p_crc, $v_pwd
        );
        // ----- Return
        //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, $v_result);
