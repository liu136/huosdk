...
class MemberController extends AdminbaseController {
    ...
    function index() {
        $this->_mem_status();
        $this->_game(true, null, null, null, null, null,false);
        $this->_agents();
        $this->_mList();
    ...
    public function add_other_account(){
        $app_id=I('app_id');
        $agent_id=I('agent_id');
        $account_str=trim(I('gameaccount'));
        $res=$this->insert_db($account_str,$app_id,$agent_id);
        if ($res) {
            $this->success("设置成功");
        } else {
    ...
    public function insert_db($account_str,$app_id,$agent_id){
        foreach (explode(',',$account_str) as $account) {
            $username=$this->get_rand_account();
            $param=[
                'username'=>$username,
                'password' => pw_auth_code($username, C("AUTHCODE")),
                'pay_pwd' =>pw_auth_code($username, C("AUTHCODE")),
                'nickname'=> $username,
                'app_id' =>$app_id,
                'agent_id'=>$agent_id,
                'gameaccount'=>$account,
            ...
            $id = M('members')->where(array('username'=>$username,'app_id'=>$app_id,'agent_id'=>$agent_id))->getField("id");
            if (!empty($id) && $id > 0) {
                return $this->insert_db($account,$app_id,$agent_id);
            } else {
                $res = M('members')->add($param);
            }
        }
        return $res;
    }
    //获取随机账号
    public  function get_rand_account(){
        $randstr=str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghigklmnopqrst1234567890');
     ...
     */
    function _mList() {
        $account_type=I('is_account');
        $gameaccount = I('gameaccount');
        $username = I('username');
        ...
        if(!empty($gameaccount) && $gameaccount != ''){
            $where['gameaccount'] = array("like", "%$gameaccount%");
            $this->assign('gameaccount', $gameaccount); 
        }
        if(!empty($account_type) && $account_type != ''){
            $where['gameaccount'] = array("neq", '');
...
//		$where_arr = array();
        ...
        $hs_where_obj->get_simple($where, "id", "id");
