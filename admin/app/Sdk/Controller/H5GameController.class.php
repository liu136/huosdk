...
use Sdk\Controller\BuildH5GameApkController;

class H5GameController extends AdminbaseController {
    protected $game_model, $gc_model, $gv_model, $cp_model,$apk_model;
    ...
    public function index() {
        $this->_game(false, null, null, 2, null, 2, true);
        $this->_game_from();
        $this->_game_status();
        $this->_gList();
    ...
    public function _gList($is_delete = false) {
        ...
        if (empty($_classify)) {
            $where_ands['_string'] = "(g.classify=2 OR g.classify BETWEEN 300 AND 399)";
        } else {
    ...
    public function edit() {
        ...
        if ($app_id > 0) {
            ...
            if ('2' == substr($gamedata['classify'], 0, 1)) {
                $has_and = '1';
            }
        ...
        if (IS_POST) {
            ...
            if ($this->game_model->create($game_data)) {
                ...
                if ($app_id > 0) {
                    ...
                    $this->success("添加成功！", U("H5Game/index"));
                }
            } else {
...
// 	                $this->gc_model->where(array('app_id'=>$app_id))->save($gc_data);
            $this->success("更新成功！", U("H5Game/index"));
// 	            }
//	        }else{
//	            $this->error($this->game_model->getError());
//	        }
   ...
   /**
     * 添加游戏入口
     */
    public function addgameurl() {
        $appid = I("appid");
        $games = $this->game_model->where("id = %d", $appid)->find();
        if ($games) {
            $this->assign("games", $games);
        } else {
            $this->error("请生成参数对接后,再添加地址");
        }
        $this->assign("games", $games);
        $this->display();
    }
   ...
   /**
     * 渠道添加游戏入口
     */
    public function addgameurl_post() {
        $appid = I("appid");
        $gameurl = I("post.gameurl", "", "trim");
        if (empty($gameurl)) {
            $this->error("请填写游戏入口地址");
        }
        $checkExpressions = '|^http://|';
        $httpsExpressions = '|^https://|';
        if (false == preg_match($checkExpressions, $gameurl) && false == preg_match($httpsExpressions, $gameurl)) {
            $this->error("请输入正确的游戏入口地址http://或者https://开头");
        }
        $g_data['id'] = $appid;
        $g_data['update_time'] = time();
        $g_data['gameurl'] = $gameurl;
        $rs = $this->game_model->where(array('id' => $appid))->save($g_data);
        if (false != $rs) {
            $this->success("添加游戏入口成功！", U("H5Game/index"));
            exit;
        } else {
            $this->error("添加游戏入口失败！");
        }
    ...
    /** 设置游戏打包参数 */
    public function set_apk_param(){
        $appid = I("appid");
        $games = $this->game_model->where("id = %d", $appid)->find();
        $apk = $this->apk_model->where("appid = %d", $appid)->find();
        $this->assign("apk", $apk);
        $this->assign("games", $games);
        $this->display();
    }
    ...
    /** 设置游戏打包参数 */
    public function set_apk_param_post(){
            $appid = I('post.appid/d', 0);
            $icon_path =I('post.thumb/s', '');
            $pack_name =I('post.packName/s', '');
            $keystore =I('post.keystore/s', '');
            $aliaskey =I('post.aliaskey/s', '');
            $validity =I('post.validity/s', '');
            if (empty($pack_name)) {
                $this->error("请填写应用包名!");
            }
            if (empty($icon_path)) {
            ...
            if( (!empty($aliaskey) || !empty($keystore) || !empty($validity)) && (empty($aliaskey) || empty($keystore) || empty($validity))
            ){ 
                $this->error("签名参数请填写完整,否则请全部留空使用默认参数!");
            }
            $apk =  $this->apk_model->where("appid = %d", $appid)->find();
            $apk_data = array(
                            'icon'=>$icon_path,
            ...
            if($apk){
                 $rs = $this->apk_model->where("appid = %d", $appid)->save($apk_data);
            }else{
                $apk_data['appid'] =  $appid;
                $rs = $this->apk_model->add($apk_data);
            }

           if (false != $rs) {
                $this->success("修改成功！", U("H5Game/index"));
                exit;
            } else {
                $this->error("修改失败！");
            }
     ...
     * 修改游戏回调
     */
    public function editgameurl_post() {
        $appid = I("appid");
        $gameurl = I("post.gameurl", "", "trim");
        if (empty($gameurl)) {
            $this->error("请填写回调地址");
    ...
    public function addpackageurl() {
        ...
        $game = $this->game_model->where("id = %d", $appid)->find();
        if (empty($apk)) {
            $this->error("请生成参数");
        }


        $url = DOWNIP."/h5BuildApk.php";
        $data_string = array( 
                             'aliaskey' => $apk['aliaskey'], 
                             'keystore' => $apk['aliaskey'], 
                             'password' => KEYSTORE_PASSWD,
                             'aliaspwd' => KEYSTORE_PASSWD, 
                             'packName' => $apk['package_name'], 
                             'gameName' => $game['name'],
                             'validity' => $apk['validity'],
                             'appid'    => $appid,
                             'time'     => time(),
                             'icon'     => $apk['icon'],
                            );
        ...
        $data_string['sign'] = md5(md5($encodeParam).'guangnaiwl123');
        $data_string = json_encode($data_string); 
        $packageurl=http_post_data($url, $data_string);

        $checkExpressions = '|^http://|';
         if (false == preg_match($checkExpressions, $packageurl)) {
            $this->error("生成失败 : {$packageurl}");
        }

        //若存在则更新地址
        $games['packageurl'] = $packageurl;
        $this->gv_model->where(array('app_id' => $appid))->save($games);
        $this->success("添加成功！", U("H5Game/index"));
    }
    ...
    public function delGame() {
        ...
        if ($rs) {
            $this->success("删除成功", U("H5Game/delindex", array('appid' => $id)));
            exit;
        }
        $this->error('删除失败.');
    }
    ...
    public function restoreGame() {
        ...
        $rs = $this->game_model->where("id = %d", $id)->save($data);
        if ($rs) {
            $this->success("还原成功", U("H5Game/index", array('appid' => $id)));
            exit;
        }
        $this->error('请求失败.');
    }
    ...
    public function set_status() {
        ...
        $rs = $this->game_model->where("id = %d", $id)->save($data);
        if ($rs) {
            $this->success("状态切换成功", U("H5Game/index", array('appid' => $id)));
            exit;
        } else {
            $this->error('状态切换失败.');
        }
