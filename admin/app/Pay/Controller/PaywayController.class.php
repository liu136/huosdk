...
class PaywayController extends AdminbaseController {
    ...
    public function index() {
        ...
        foreach ($ways as $key => $val) {
            $sql = "select * from c_".$val['payname']."history a WHERE
				NOT EXISTS (SELECT 1 FROM c_".$val['payname']
                   ."history b WHERE a.cid=b.cid AND a.create_time<b.create_time AND b.cid=%d) AND a.cid=%d";
    ...
    public function _addAlipay($cid) {
        ...
        if (empty($alipay['email'])
            || empty($alipay['partner'])
            || empty($alipay['alikey'])
            || empty($alipay['alipay_public_key'])
            || empty($alipay['rsa_private_key'])
            || empty($alipay['rsa_public_key'])
        ) {
            return false;
        }
        if (empty($filepath)) {
    ...
    public function _addYeepay($cid) {
        ...
        if (empty($yeepay['merchantaccount']) || empty($yeepay['yeepaypublickey'])
            || empty($yeepay['merchantprivatekey'])
            || empty($yeepay['merchantpublickey'])
        ) {
            return false;
        }
        if (empty($filepath)) {
    ...
    public function _checkHistory($table, $where = null) {
        $model = M();
        $cid = C('CLIENTID');
        $sql = "select * from c_".$table."history a WHERE
				NOT EXISTS (SELECT 1 FROM c_".$table
               ."history b WHERE a.cid=b.cid AND a.create_time<b.create_time AND b.cid=%d) AND a.cid=%d";
        $info = $model->query($sql, $cid, $cid);
