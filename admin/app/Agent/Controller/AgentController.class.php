...
class AgentController extends AdminbaseController {
    ...
    function edit() {
        ...
        if (2 < $this->role_type) {
            if ($adminid != $user['id'] && $adminid != $user['ownerid']) {
                $this->error("无权限");
            }
        }
        if (3 == $this->role_type) {
            if ($adminid == $user['id']) {
                $this->error("亲，您不能在此编辑自己的账号");
            }
            $this->_roles(4, false);
        } else {
            $this->_roles(3, false);
    ...
    function _getAgentdata() {
        ...
        $field
            = "`date`, `agentid`, sum(`user_cnt`) user_cnt, sum(`summoney`) summoney, sum(`paycnt`) paycnt,
                sum(`regpaycnt`), sum(`sumregmoney`) sumregmoney, sum(`reg_cnt`) reg_cnt";
        $sumfield
            = "sum(`user_cnt`) `user_cnt`,sum(`summoney`) `summoney`,sum(`paycnt`) `paycnt`,sum(`regpaycnt`) `regpaycnt`,sum(`sumregmoney`) `sumregmoney`,sum(`reg_cnt`) `reg_cnt`";
        $items = $paymodel
            ->field($field)
            ->where($where)
            ->group('date, agentid')
...
//         $this->assign("todaypays", $todayitem);
    ...
    function _getAgentreten() {
        ...
        if ($bflagstart) {
            $sql
                = "SELECT `date`,g.name,u.user_nicename,`second_cnt`, `third_cnt`, `fourth_cnt`, `fifth_cnt`, `sixth_cnt`, `seventh_cnt`, `fifteenth_cnt`, `thirtieth_cnt`
					...
					UNION ALL
					SELECT `date`, `agentid`, `appid`, `user_cnt`, `summoney`, `paycnt`, `regpaycnt`, `sumregmoney`, `reg_cnt`, `second_cnt`, `third_cnt`, `fourth_cnt`, `fifth_cnt`, `sixth_cnt`, `seventh_cnt`, `fifteenth_cnt`, `thirtieth_cnt` FROM "
    ...
    function setdiscout() {
        $userid = I("id");
        $type = $this->users_model->where(array("id" => $userid))->getField("is_discount");
        //is_discount中，1为打折，2为不打折
        $data['is_discount'] = 2;
        if ($type == 2) {
            $data['is_discount'] = 1;
        }
        $rs = $this->users_model->where(array("id" => $userid))->data($data)->save();
        if ($rs) {
            $this->success("折扣设定成功。");
            exit;
        }
