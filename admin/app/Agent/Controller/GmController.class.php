...
class GmController extends AdminbaseController {
    ...
    public function _gmList($is_delete) {
        ...
        $items = $this->gm_model
            ...
            ->select();
        $agentfield = "app_id,sum(remain) aremain,count(*) acount";
        $agentdate = M('gm_agent')->group('app_id')->getField($agentfield, true);
        $mfield = "app_id,sum(remain) mremain,count(*) mcount";
        $memdate = M('gm_mem')->group('app_id')->getField($mfield, true);
        $this->assign("agentdate", $agentdate);
        $this->assign("memdate", $memdate);
    ...
    public function add() {
        $where['g.status'] = 2;
        $where['g.is_delete'] = 2;
        $app_ids = M('gm')->where(array('is_delete' => 2))->getField('app_id', true);
    ...
    function _officeList() {
        ...
        if (!empty($username)) {
            $string = "(m.username = '".$username."') OR (au.user_login = '".$username."')";
            array_push($where_ands, $string);
        }
        if ('七日' == $_POST['submit']) {
