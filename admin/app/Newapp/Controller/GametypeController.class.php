        ...
        foreach ($result as $n => $r) {
            ...
            $result[$n]['str_manage'] = '<a href="'.U(
                    "Gametype/edit", array("id" => $r['id'])
                                        ...
                                        ).'">删除</a> | <a class="js-ajax-submit" href="'.U(
                                            "Gametype/setindex", array("id" => $r['id'], "menuid" => I("get.menuid"))
                                        ).'">'.$str.'</a>';
            if (0 == $r['parentid']) {
                $result[$n]['str_manage'] = '<a href="'.U(
...
//        $this->display();
    ...
    public function edit_post() {
        if (IS_POST) {
            ...
            if ($gt_data['id'] > 0) {
                ...
                $_check_map['name'] = $gt_data['name'];
                $_check_map['id'] = array("neq",$gt_data['id']);
    ...
    function setindex() {
        $id = I('id');
        if ($id > 0) {
            $record = M('game_type')->getById($id);
            if($record['listorder'] == 1000) {
                $data = [
                    'listorder' => 0,
                ];
            }else{
                $data = [
                    'listorder' => 1000,//暂定为1000时在首页显示
                ];
                $count = M('game_type')->where($data)->count();
                if ($count >= 4) {
                    $this->error("只能有4个标签在首页展示");
                    exit;
                }
            }
            M('game_type')->where(['id' => $id])->save($data);
            $this->success("设置成功！", U("Newapp/Gametype/index"));
        }
    }
}
