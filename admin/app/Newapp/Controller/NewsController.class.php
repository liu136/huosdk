...
class NewsController extends AdminbaseController {
    ...
    function index() {
        ...
        $this->assign("gameclassify", $gameclassify);
        $where_ands = array(
            " post_status>0"
        );
        $fields = array(
        ...
        $count = $this->posts_model->alias('p')->join(
            "left join ".C('DB_PREFIX')."game g ON p.app_id=g.id"
        )->join(
            "left join ".C('DB_PREFIX')."game_class gc ON g.classify=gc.id"
        )->where($where)->count();
        $page = $this->page($count, 20);
        $posts = $this->posts_model->alias('p')->field("p.*, g.name gamename,gc.realname gameclassify")->join(
            "left join ".C('DB_PREFIX')."game g ON p.app_id=g.id"
        )->join(
            "left join ".C('DB_PREFIX')."game_class gc ON g.classify=gc.id"
        )->where($where)->limit(
            $page->firstRow.','.$page->listRows
        )->order("post_modified desc")->select();
//        print_r($posts);exit;
...
//             $_POST['smeta']['thumb'] = sp_asset_relative_url($_POST['smeta']['thumb']);
    ...
    public function settop() {
        ...
        if (!empty($ids)) {
            ...
            M('posts')->where(['id' => ['in', $ids]])->save($data);
            $this->success("置顶成功");
        }
    }
