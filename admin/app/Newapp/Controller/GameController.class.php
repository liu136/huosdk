...
//         array_push($where_ands, "g.is_app  = 2");
    ...
    public function formatIsNew(&$items) {
        $data = array("0" => "否","1" => "否", "2" => "是");
        $data1 = array("1" => "否", "2" => "是");
        foreach ($items as $key => $value) {
            $items[$key]['is_new_txt'] = $data[$value['is_new']];
            $items[$key]['is_welfare_txt'] = $data1[$value['is_welfare']];
        }
    }
...
//             }
            ...
            if ($this->game_model->create($game_data)) {
                ...
                /* 插入游戏类型 */
                if ($app_id > 0) {
                    $game_data['app_key'] = md5(rand(1, 90000).md5($game_data['pinyin']).uniqid().'##');
...
//                     $info_data['mobile_icon'] = $game_data['icon'];
    ...
    public function edit() {
        ...
        if ($app_id > 0) {
            ...
            $this->gameStatus(1);
            /*join($join)->*/
            $gamedata = $this->game_model->where(
                array(
                    'id' => $app_id
                )
            )->find();
...
//                     $info_data['mobile_icon'] = $game_data['icon'];
    ...
    function check() {
        ...
        if (isset($_POST['ids']) && $_GET["online"]) {
            ...
            foreach ($ids as $v) {
               ...
               // if (empty($packageurl)) {
                //    $this->error("请填写游戏下载地址", U('Sdk/Game/edit', array('id' => $id)));
                //}
            }
            $app_ids = join(",", $_POST['ids']);
            if ($this->game_model->where("id in ($app_ids)")->save($data) !== false) {
    ...
    public function gclassify() {
        ...
        $this->assign("gclassify", $_g_class);
    ...
    function welfare() {
        if (isset($_POST['ids']) && $_GET["welfare"]) {
            $data["is_welfare"] = 2;
            $app_ids = join(",", $_POST['ids']);
            if ($this->game_model->where("id in ($app_ids)")->save($data) !== false) {
                $this->success("设置公益成功！");
        ...
        if (isset($_POST['ids']) && $_GET["unwelfare"]) {
            $data["is_welfare"] = 1;
            $app_ids = join(",", $_POST['ids']);
            if ($this->game_model->where("id in ($app_ids)")->save($data) !== false) {
                $this->success("取消公益成功！");
