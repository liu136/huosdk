...
class SettleController extends AdminbaseController {
    ...
    public function operator_check() {
        ...
        $_field
            = "awr.*,u.user_nicename,pa.remain as balance,u.user_login,u.linkman,u.qq,u.mobile,
        am.link_man d_linkman,am.bankname d_bankname,
        am.branchname d_branchname,am.banknum d_banknum,am.cardholder d_cardholder";
        $items = $model
            ->field($_field)
            ->alias('awr')
            ...
            ->join("LEFT JOIN ".C('DB_PREFIX')."agent_man am ON am.agent_id=awr.agent_id")
            ->where($where)
            ->order("awr.id desc")
    ...
    public function check() {
        ...
        if (isset($_GET['agent_id']) && !empty($_GET['agent_id'])) {
            $where = array("awr.agent_id" => $_GET['agent_id']);
        }
        $count = $model->alias('awr')->where($where)->count();
        $page = $this->page($count, $this->row);
        $_field
    ...
    public function okrecord() {
        ...
        $where["awr.status"] = "3";
        if (isset($_GET['agent_id']) && !empty($_GET['agent_id'])) {
            $where["awr.agent_id"] = $_GET['agent_id'];
        }
        $count = $model->alias('awr')->where($where)->count();
        $page = $this->page($count, $this->row);
        $_field
            = "awr.*,u.user_nicename,u.user_login,u.linkman,u.qq,u.mobile,
        am.link_man d_linkman,am.bankname d_bankname,
        am.branchname d_branchname,am.banknum d_banknum,am.cardholder d_cardholder";
        $items = $model
            ->field($_field)
