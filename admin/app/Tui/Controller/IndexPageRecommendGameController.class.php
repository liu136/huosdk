...
class IndexPageRecommendGameController extends AdminbaseController {
    ...
    public function getHotGameLists() {
        $model = M('options');
        $data = $model->where(array("option_name" => "index_page_recommend_games"))->getField("option_value");
        if (!$data) {
            return array();
        }
    ...
    public function hot_post() {
        ...
        $aa = M('options')->where(array("option_name" => "index_page_recommend_games"))->find();
        if (M('options')->where(array("option_name" => "index_page_recommend_games"))->find()) {
            M('options')->where(array("option_name" => "index_page_recommend_games"))->setField("option_value", $new_lists);
        } else {
            $adddata["option_name"] = "index_page_recommend_games";
