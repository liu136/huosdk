...
//        $where = join(' AND ', $where);
    ...
    public function getTotal($where_extra) {
        $_field = ""
                  ."sum(dag.user_cnt) as sum_new_user_cnt,"
                  ."sum(dag.reg_cnt) as sum_active_reg_cnt,"
                  ."sum(dag.day2) as sum_day2,"
                  ."sum(dag.day3) as sum_day3,"
                  ."sum(dag.day7) as sum_day7,"
                  ."sum(dag.sum_money) as sum_charge_amount,"
                  ."sum(dag.sum_reg_money) as sum_reg_money,"
                  ."sum(dag.pay_user_cnt) as sum_pay_user_cnt,"
                  ."sum(dag.reg_pay_cnt) as sum_reg_pay_cnt"
                  ;
...
//            echo $agent_id;
            $all_app_data = M('day_agentgame')
                ...
                ->field(
                  ...
                  ."sum(dag.user_cnt) as user_cnt,"
                  ."sum(dag.reg_cnt) as reg_cnt,"
                  ."concat(ifnull(round(sum(dag.day2/dag.reg_cnt),2),0),'%') as day2,"
                  ."concat(ifnull(round(sum(dag.day3/dag.reg_cnt),2),0),'%') as day3,"
                  ."concat(ifnull(round(sum(dag.day7/dag.reg_cnt),2),0),'%') as day7,"
                  ."sum(dag.sum_money) as sum_money,"
                  ."sum(dag.sum_reg_money) as sum_reg_money,"
                  ."sum(dag.pay_user_cnt) as pay_user_cnt,"
                  // ."sum(dag.reg_pay_cnt) as reg_pay_cnt,"
                  ."concat(ifnull(round(sum(dag.pay_user_cnt/dag.user_cnt),2),0),'%') as sum_apru,"
                  ."concat(ifnull(round(sum(dag.reg_pay_cnt/dag.reg_cnt),2),0),'%') as reg_apru,"
                  ."ifnull(round(sum(dag.sum_money/dag.pay_user_cnt),2),0) as arppu,"
                  ."ifnull(round(sum(dag.sum_reg_money/dag.pay_user_cnt),2),0) as reg_arppu,"
                  ."ifnull(round(sum(dag.sum_money/dag.reg_cnt),2),0) as ltv"
                )
                ->where(array("dag.agent_id" => $agent_id, "dag.date" => $date))
                ->find();
