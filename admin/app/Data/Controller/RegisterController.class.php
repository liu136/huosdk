...
class RegisterController extends AdminbaseController {
    ...
    public function regList() {
        ...
        foreach ($items as $key => $val) {
            ...
            $logintime = $login->where("mem_id =%d", $val['id'])->order("id DESC")->limit(1)->field('login_time')
                               ->select();
            $logincount = $login->where("mem_id=%d and login_time>'%s'", array($val['id'], $two))->count("id");
            $items[$key]['login_time'] = isset($logintime[0]['login_time']) ? date(
                'Y-m-d H:i:s', $logintime[0]['login_time']
            ) : "--";    //最近登录时间
            $items[$key]['logincount'] = isset($logincount) ? $logincount : "--";    //最近登录次数
            $sql
                = "SELECT SUM(amount) AS money,COUNT(id) AS paycount,agent FROM c_pay WHERE username='%s' and status=1";
            ...
            $agentdata = $agentlist->field('c_users.user_login,c_users.user_nicename')->join($join)->where(
                "c_agentlist.agentgame='%s'", $val['agentgame']
            )->select();
            $items[$key]['owner'] = isset($agentdata[0]['user_login']) ? $agentdata[0]['user_login']
                : "--";            //渠道专员
            $items[$key]['agentname'] = isset($agentdata[0]['user_nicename']) ? $agentdata[0]['user_nicename']
                : "--";    //渠道名称
        }
        $result["rows"] = $items;
        if ($result["total"] == 0) {
        ...
        if ($_REQUEST['downloads'] == 1) {
            $list = $model->where($where, $where_arr)->order("id DESC")->field('id,username,imei,agentgame,reg_time')
                          ->select();
            $title = "新注册用户列表";
    ...
    public function loginList() {
        ...
        foreach ($items as $key => $val) {
            ...
            $items[$key]['game'] = $game[$val['gameid']];
            $join = "c_users on c_agentlist.agentid=c_users.id";
            $agentdata = $agentmodel->field('c_users.user_login')->join($join)->where(
                "c_agentlist.agentgame='%s'", $val['agentgame']
            )->select();
        ...
        //下载
        if ($_REQUEST['downloads'] == 1) {
            $list = $model->where($where, $where_arr)->order("id DESC")->field('id,mem_id,agentgame,login_time')
                          ->select();
            $title = "登录用户统计";
