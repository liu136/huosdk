...
class GamedesController extends AdminbaseController {
    ...
    public function gList() {
        ...
        //获取游戏内容
        $items = $this->game_model->where($map)->order('listorder ASC ,id DESC')->limit(
    ...
    public function addGamedes_post() {
        ...
        $rs = $this->game_model->where(array("id" => $app_id))->save($g_data);
        if ($rs) {
...
// 		}
        ...
        $version_id = M('gameVersion')->where($map)->order('create_time DESC')->getField(
            'id'
        ); //添加order排序是为了app有多个版本是修改最新的版本号
        $gversions = M('gameVersion')->where(array("id" => $version_id))->save($appversions);
        //保存游戏的信息
        $rs = $game_model->where(array("id" => $checkappid))->data($game_data)->save();

        //游戏标签

        $type_data['app_id'] = $checkappid;
        $type_data['type_id'] = $game_data['type'];
        $gtmodel = M('game_gt');
        $gtmodel->where(array('app_id' => $checkappid))->delete();
        $gtmodel->data($type_data)->add();

        if ($rs) {
            $gameInfo = M("gameInfo");
            //获取游戏的ID
            $app_id = $gameInfo->where(array("app_id" => $checkappid))->getfield('app_id');
            //游戏存在则执行保存，不存在则执行插入
            if ($app_id) {
                $result = $gameInfo->where(array("app_id" => $checkappid))->data($infodata)->save();
            } else {
                $result = $gameInfo->where(array("app_id" => $checkappid))->data($infodata)->add();
            }
            $this->success("编辑成功");
        } else {
            $this->error("编辑失败。");
