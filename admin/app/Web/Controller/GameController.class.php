...
class GameController extends AdminbaseController {
    ...
    public function serverNewList() {
        $this->redirect('Newapp/Gameserver/index');
        exit;
        //获取页码和每页显示条数
        $page = I('p');   //获取页码
        ...
        $items = $model->field($field)->where($map)->order("id DESC")->limit($page->firstRow.','.$page->listRows)
                       ->select();
        $gameDao = M('game');
    ...
    public function addServer() {
        $games = $this->game_model->field('id,name')->where("status in (1,2)")->order("id desc")->select();
        $this->assign("games", $games);
        $this->display();
    }
...
//		}
    ...
    public function gameList() {
        redirect(U('Newapp/Game/index'));
        exit;
    }

    //开服管理
    public function Gameserver() {
        redirect(U('Newapp/Gameserver/index'));
        exit;
    }

    //开测管理
    public function Gametest() {
        redirect(U('Newapp/Gametest/index'));
        exit;
    }

    //游戏标签
    public function Gametype(){
        redirect(U('Newapp/Gametype/index'));
        exit;
    }
}
