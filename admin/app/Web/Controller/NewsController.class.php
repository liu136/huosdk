...
class NewsController extends AdminbaseController {
    ...
    public function findList() {
        $this->_game(true, 2, 2,null,null,null);
        $this->nList();
        $this->display();
    }
    ...
    public function addNews_post() {
        ...
        if (empty($news_data['title']) || empty($content_data['content'])) {
...
//        $news_data['create_time'] = time();                 //创建时间
        ...
        $content = M("webContent");
        $check = $content->where("id = '%s'", $id)->find();
        if (!empty($check)) {
            $rs1 = $content->where("id = '%s'", $id)->data($content_data)->save();
        } else {
            $content_data['id'] = $id;
            $rs1 = $content->data($content_data)->add();
        }
        if (false !== $rs) {
