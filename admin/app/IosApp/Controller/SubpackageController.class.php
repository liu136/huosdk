...
// 	    }
    ...
    public function _aList() {
        $page = I('p');   //获取页码
        $gameid = I('app_id/d', C('IOSAPP_APPID'));
    ...
    public function subpackage() {
        if (IS_POST) {
            //获取参数
            $app_id = I('post.app_id/d', C('IOSAPP_APPID'));
...
//	            $ag_info['agent_rate'] = $rate? $rate:0;
            ...
            if ($ver_id <= 0) {
                ...
                $ver_id = M('game_version')->where($map)->order('id desc')->getfield('id');
            }
            if ($ver_id <= 0) {
                $this->error("参数错误ver");
            }
            $this->_do_package($ag_info['id'], 1, $ver_id);
        }
    ...
    function _do_package($ag_id, $option = 1, $ver_id) {
        if (empty($ag_id)) {
        ...
        if (0 < $return_content) {
            $updatedata['url'] = $init.'/'.$ag_info['agentgame'].".apk";;
    ...
    public function updatepackage() {
        ...
        $this->_do_package($ag_id, 2, $ver_id);
    }
