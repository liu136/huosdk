...
class AppapiController extends Controller {
    ...
    public function index() {
        ...
        $indexDao = M('appindex');
        $indexlist = $indexDao->field("lunbotu,onegameid,twgameid,thrgameid,fourgameid")->where(" isdelete='0' ")
                              ->order('id DESC')->select(); //查询游戏类型
    ...
    public function jflog() {
        ...
        $lists = $jifenalls->field("jifen as a,create_time as b,beizhu as c")->where("username='".$username."'")->limit(
            $offset.','.$pagesize
        )->select();
    ...
    public function lblog() {
        ...
        $pagesize = 10;
        $jifenall = M('ttbpay');
        $list = $jifenall->field("ttb as a,paytypeid as b,create_time as c,status as d")->where(
            "username='".$username."'"
        )->select();
        $myrow = mysql_fetch_array($list);
        $numrows = $myrow[0];
        ...
        $offset = $pagesize * ($page - 1);
        $jifenalls = M('ttbpay');
        $lists = $jifenalls->field("ttb as a,paytypeid as b,create_time as c,status as d")->where(
            "username='".$username."'"
    ...
    public function login() {
        ...
        $password = pw_auth_code($password);
        $model = M('members');
        $userlist = $model->field("username as b")->where("username='".$username."' AND password='".$password."'")
                          ->select();
        $userlist = $userlist[0];
        $checkloging = 0;
        if (!empty($userlist['b'])) {
    ...
    public function updatePwd() {
        ...
        $ckupdate = 0;
        $model = M('members');
        $userlist = $model->field("username as b")->where("username='".$username."' AND password='".$oldpwd."'")
                          ->select();
        $userlist = $userlist[0];
        if (!empty($userlist['b'])) {
    ...
    public function dhlog() {
        ...
        $loglist = $dhlog->field($field)->where("username = '".$username."' AND status='1' AND code IS NOT NULL ")
    ...
    public function tgameList() {
        ...
        $gamelist = $game->field($field)->join($join)->where($where)->order($order)->limit($offset.','.$pagesize)
                         ->select();
    ...
    public function gameList() {
        ...
        $join = "c_appgameinfo ON c_appgame.id=c_appgameinfo.id";
        $gamelist = $game->field($field)->join($join)->where($where)->order('count DESC,id DESC')->limit(
            $offset.','.$pagesize
        )->select();
        $baoming = M('app_baoming');
        $baolist = $baoming->field("id,gameid,name")->select();
    ...
    public function giftList() {
        ...
        $time = time();
        $field = "c_applibaoinfo.id as a,c_applibaoinfo.title as b,c_applibaoinfo.content as c,CONCAT('".$this->website
                 ."/upfiles/image/',c_appgame.image) AS d,c_applibaoinfo.starttime as e";
        $join = " c_appgame ON c_applibaoinfo.gameid=c_appgame.id";
        $giftlist = $libaoinfo->field($field)->join($join)->where(
            "c_applibaoinfo.isdelete=0 and c_applibaoinfo.endtime>'".$time."'"
        )->order('starttime ASC')->select();
        $arr = array();
    ...
    public function giftDetail() {
        ...
        $libaoinfo = M('applibaoinfo');
        $field
            = "c_applibaoinfo.id as a,c_applibaoinfo.gameid as b,c_applibaoinfo.title as c,c_applibaoinfo.content as d,CONCAT('"
              .$this->website
              ."/upfiles/image/',c_appgame.image) AS e,c_applibaoinfo.starttime as f,c_applibaoinfo.endtime as g";
        $join = " c_appgame ON c_applibaoinfo.gameid=c_appgame.id";
    ...
    public function jfList() {
        ...
        $field = "id as a,title as b,number as c,content as d,CONCAT('".$this->website
                 ."/upfiles/image/',ico) AS e,type as f,endtime as j,create_time as k";
        $wupinlist = $wupininfo->field($field)->where("isdelete=0")->order('id DESC')->limit($offset.','.$pagesize)
                               ->select();
        $arr = array();
    ...
    public function getMyGameList() {
        ...
        foreach ($baolist as $key => $val) {
            $libaodata = $libaoinfo->field("id,endtime")->where("gameid=".$val['a']." and isdelete=0")->order(
                "create_time DESC"
            )->limit(1)->select();
            ...
            $serverdata = $serverinfo->field("id,sertime")->where("gameid=".$val['a']." and isdelete=0")->order(
                "create_time DESC"
            )->limit(1)->select();
    ...
    public function rwList() {
        ...
        $taskmode = M('apptask');
        $field
            = "c_apptask.id as a,c_apptask.title as b,c_apptask.content as c,CONCAT($this->website,'/upfiles/image/',c_appgame.image) AS d,c_apptask.jifen as e,c_apptask.gameid as f,c_apptask.endtime as h,c_apptask.starttime as i";
        $join = " c_appgame ON c_apptask.gameid=c_appgame.id";
        $tasklist = $taskmode->field($field)->join($join)->where("c_apptask.isdelete=0")->order('c_apptask.id DESC')
                             ->limit($offset.','.$pagesize)->select();
        $arr = array();
