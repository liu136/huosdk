...
// 	    $this->assign('username', $username);
    ...
    public function give_verify() {
        ...
        //}
        $agent_id = sp_get_current_admin_id();
        $give_max_cnt = M('gm_agent')->where(array('app_id' => $_POST['app_id'], 'agent_id' => $agent_id))->getField(
            'remain'
        );
    ...
    public function give_post() {
        ...
        if (isset($action) && isset($action) == 'add') {
            ...
            $give_max_cnt = M('gm_agent')->where(array('app_id' => $data['app_id'], 'agent_id' => $data['admin_id']))
                                         ->getField('remain');
            if ($data['gm_cnt'] > $give_max_cnt) {
                $this->error($data['gm_cnt'].'超过已有游戏币额度'.$give_max_cnt.',发放失败');
            }
            ...
            if (1 == $takeclass) {
                ...
                if ($rs) {
                    $ga_model = M('gm_agent');
                    //扣除拥有数量
                    $gaars = $ga_model->where(array('agent_id' => $data['admin_id'], 'app_id' => $data['app_id']))
                                      ->setDec('remain', $data['gm_cnt']);
                    if (!$gaars) {
                        $this->error('转账失败');
                    }
