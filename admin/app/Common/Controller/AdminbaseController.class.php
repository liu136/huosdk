...
class AdminbaseController extends AppframeController {
    ...
    function _game(
        $option = true, $status = null, $is_delete = null, $is_sdk = null, $is_app = null, $classfy = "",
        $game_flag = false
    ) {
        $cates = array(
            "0" => "选择游戏",
        ...
        if ($classfy) {
            if (3 == substr($classfy, 0, 1) || 4 == substr($classfy, 0, 1)) {
                $where['_string'] = " classify=3 OR classify=4 OR (classify BETWEEN 300 AND 399)";
            } else {
    ...
    function _game_parent_type($option = true,$parent = 0) {
        $cates = array(
            "0" => "全部类型",
        );
        ...
        if($parent == 0) {
            $this->assign("gameparenttypes", $gametypes);
        }else{
            $this->assign("gametypes", $gametypes);
        }
    }
    ...
    /**
     * 根据app_id,获取app_id下的渠道id及名称
     * @param  integer $app_id [游戏id]
     * @return [array]          [渠道id=>渠道名称]
     */
    function _game_agents($app_id=0){
        $cates = array(
            "-1"=>"所有渠道",
            "0" => "官方渠道"
        );
        if(!empty($app_id)){
            $item = M('agent_game')
                ->field("gg.agent_id,u.user_nicename as agent_name")
                ->alias("gg")
                ->where(['app_id' => $app_id])
            ...
            foreach ($item as $value) {
                $cates[$value['agent_id']]=$value['agent_name'];
            }
        }
        $this->assign("agents", $cates);
    }
