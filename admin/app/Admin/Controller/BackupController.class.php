...
class BackupController extends AdminbaseController {
    ...
    public function index_post() {
        if (IS_POST || isset($_GET['dosubmit'])) {
            ...
            if (isset($_GET['type']) && $_GET['type'] == 'url') {
                $sizelimit = isset($_GET['sizelimit']) && abs(intval($_GET['sizelimit'])) ? abs(
                    intval
                    (
                        $_GET['sizelimit']
                    )
                ) : $this->error('请输入每个分卷文件大小');
                ...
                if (!is_file(
                    SITE_PATH.$this->backup_path.$this->backup_name.
                    '/tbl_queue.log'
                )
                ) {
                    //写入队列
    ...
    public function import() {
        $backup_name = isset($_GET['backup']) && trim($_GET['backup'])
            ? trim($_GET['backup'])
            :
            $this->error('请选择备份名称');
    ...
    public function del_backup() {
        if ((!isset($_GET['backup']) || empty($_GET['backup']))
            && (!isset($_POST['backup']) || empty($_POST['backup']))
        ) {
            $this->error('非法参数');
        }
        import('@.ORG.Dir');
            ...
            if ($handle = opendir(SITE_PATH.$this->backup_path)) {
                while (($file = readdir($handle)) !== false) {
                    if ($file{0} != '.' && filetype(SITE_PATH.$this->backup_path.$file) == 'dir') {
                        ...
                        $backup['total_size'] = round(
                            $this->_get_dir_size(SITE_PATH.$this->backup_path.$file) / 1024, 2
                        );
                        $backups[] = $backup;
                    }
                }
    ...
    private function _deal_result($tables, $vol, $sizelimit) {
        ...
        if (empty($tables)) {
            ...
            $link = U(
                "Backup/index", array("dosubmit"  => 1, "type" => "url", "backup_name" => $this->backup_name,
                                      "sizelimit" => $sizelimit)
            );
    ...
    private function _get_table_data($table, $pos, $sizelimit) {
        ...
        for ($i = 0; $i < $cycle_time; $i++) {
            $data = $this->_database_mod->query(
                "SELECT * FROM $table LIMIT ".($this->offset * $i + $pos).', '.$this->offset
            );
