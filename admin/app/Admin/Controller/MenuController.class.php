...
class MenuController extends AdminbaseController {
    ...
    public function index() {
        ...
        foreach ($result as $n => $r) {
            ...
            $result[$n]['str_manage'] = '<a href="'.U(
                    "Menu/add", array("parentid" => $r['id'], "menuid" => $_GET['menuid'])
                ).'">添加子菜单</a> | <a target="_blank" href="'.U(
                                            "Menu/edit", array("id" => $r['id'], "menuid" => $_GET['menuid'])
                                        ).'">修改</a> | <a class="J_ajax_del" href="'.U(
                                            "Menu/delete", array("id" => $r['id'], "menuid" => I("get.menuid"))
                                        ).'">删除</a> ';
    ...
    public function syncMenuRule() {
        $_menus = $this->menu_model->select();
        $_miss_rule = "";
        foreach ($_menus as $_k => $_v) {
            ...
            if (!$find_rule) {
                ...
                $this->auth_rule_model->add($_auth_data);//type 1-admin rule;2-user rule
                $_miss_rule .= $name.'<br>';
            } else {
                continue;
            }
        ...
        if (empty($_miss_rule)) {
            $this->success("同步成功！", U('Index/index'));
        } else {
            echo "同步成功！添加rule:".$_miss_rule;
        }
    }
        ...
        if (IS_POST) {
            if ($this->menu_model->create()) {
                if ($this->menu_model->add() !== false) {
                    ...
                    $find_rule = $this->auth_rule_model->where($mwhere)->find();
                    if (!$find_rule) {
                        $this->auth_rule_model->add(
                            array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu_name)
            ...
            if ($this->menu_model->create()) {
                if ($this->menu_model->save() !== false) {
                    ...
                    if (!$find_rule) {
                        ...
                        $this->auth_rule_model->where($mwhere)->save(
                            array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu_name)
                        );//type 1-admin rule;2-user rule
                    }
                    $this->success("更新成功！");
    ...
    private function _import_menu($menus, $parentid = 0, &$error_menus = array()) {
        foreach ($menus as $menu) {
            ...
            if (!$find_rule) {
                $this->auth_rule_model->add(
                    array("name" => $name, "module" => $app, "type" => "admin_url", "title" => $menu['name'])
                );//type 1-admin rule;2-user rule
            } else {
                $this->auth_rule_model->where($mwhere)->save(
                    array("module" => $app, "type" => "admin_url", "title" => $menu['name'])
                );//type 1-admin rule;2-user rule
            }
                            ...
                            if (!(strpos($mf, ".") === 0)) {
                                ...
                                foreach ($methods as $a) {
                                    if (!(strpos($a, "_") === 0) && !(strpos($a, "spmy_") === 0)) {
                                        ...
                                        if (!$find_rule) {
                                            $this->auth_rule_model->add(
                                                array("name"  => $name, "module" => $g, "type" => "admin_url",
                                                      "title" => "")
                                            );//type 1-admin rule;2-user rule
                                        }
                                    }
