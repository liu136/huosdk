...
class LogController extends AdminbaseController {
    ...
    public function showlog(){
        $datalist = array();
        foreach (glob(ERROR_PATH.'*.log') as $file){
            $datalist[$file]=basename($file);
        }
        $this->assign("files", $datalist);
        $this->display();
    }
    //后台错误文件显示
    public function show_err_detail(){
        $file_anme=I('file_name');
        $content = nl2br(file_get_contents(ERROR_PATH.'/'.$file_anme));
        exit($content);
    }
}