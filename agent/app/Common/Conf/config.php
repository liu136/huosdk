...
// 代理配置文件引入
if (file_exists(SITE_PATH."conf/agent/config.php")) {
    $agent_config = include SITE_PATH."conf/agent/config.php";
} else {
    $agent_config = array();
}
// 路由配置文件
...
$configs = array(
    ...
    'SP_TMPL_PATH'           => SITE_PATH.'themes/',       // 前台模板文件根目录
    ...
    'SP_ADMIN_STYLE'         => 'flat',
    'SP_ADMIN_TMPL_PATH'     => SITE_PATH.'agent/themes/',       // 各个项目后台模板文件根目录
    'SP_ADMIN_DEFAULT_THEME' => 'simplebootx',       // 各个项目后台模板文件
    //    'SP_ADMIN_TMPL_ACTION_ERROR' 	=> 'Agent/error.html', // 默认错误跳转对应的模板文件,注：相对于后台模板路径
    //    'SP_ADMIN_TMPL_ACTION_SUCCESS' 	=> 'Agent/success.html', // 默认成功跳转对应的模板文件,注：相对于后台模板路径
    ...
    'COOKIE_DOMAIN'          => H5_DOMAIN,  // cookie域名
    "COOKIE_EXPIRE"          => "3600",
    ...
    'TMPL_PARSE_STRING'      => array(
        ...
        "__ICONPREFIX__" => "/upload/"
    )
);
return array_merge(
    $configs, $routes, $db, $agent_config, $runtime_config, $alipay_config, $company_config, $setting_config
);
